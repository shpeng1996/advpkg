...
## [2026-09-22] collect | 每日自動蒐集 — 先進封裝最新動態（三軌執行：新聞 + 專利 + 論文）
- 搜尋查詢數：新聞 5 組 / 專利 7 組（CQL；5 組 HTTP 200、2 組 404 已依 spec 簡化重試，其中 1 組重試成功）/ 論文 4 組（OpenAlex filter，全部 200）
- 成功抓取：14 篇（articles 4, patents 5, papers 5, reports 0）
- 失敗/跳過：約 95 篇（已收錄 4：ASE 310mm PLP 新聞同一事件已於 2026-05-30 收錄、HyperFRAME AMAT 篇已於 2026-09-20 收錄、EVG ECTC 2026 篇已收錄、SanDisk family 99884234 已收錄；付費牆／403 共 1：**Wiley Advanced Science（Kang, CMP of plasma-modified Cu/polymer interfaces, 10.1002/advs.202512611）連續第二輪 403**，⚠ 仍為最值得取得的單一文獻；主動棄收（純配方／無封裝架構內容）約 12：OpenAlex 與 OPS 檢出之 CMP 漿料配方案（YCCHEM、安集微、Samsung histidine、KCTECH、LISEN NOCO 等）；重複家族棄收 3：SanDisk D2W 同日同標題之 family 99884269 / 99884271 / 99889249（與已收錄之 99884234 摘要近乎相同，邊際價值低）；低相關約 75：OpenAlex 四組 100 筆去重後多為玻璃帷幕牆、瀝青路面感測器、天線、Zenodo 自行上傳之未同行評審稿件等非半導體封裝命中）
- 資料源狀態：WebSearch ACTIVE ✓ | EPO OPS ACTIVE ✓ | OpenAlex ACTIVE ✓（OPS 9 次呼叫／上限 10：1 auth + 7 search + 1 biblio detail，無 403/429；OpenAlex 4 組全部 200，無 429）
- 新增 wiki 頁面：**15 頁**（來源摘要頁 14 + 實體頁 1）
  - 來源摘要 14 頁，依**發表日**命名並以程式依發表日插入 `index.md`，逐筆驗證前後序
  - `wiki/entities/hanmi.md`（⭐ 結清列管四輪的實體頁空缺；觸發點：本輪新聞軌取得其混合接合機完整時程）
- 更新 wiki 頁面：10 頁
  - `wiki/technologies/hybrid-bonding.md`（⭐⭐⭐ dishing 雙向失效與 3–5 nm 控制；⭐⭐ 限制鏈第四環候選；⭐⭐⭐ 第一件「規避混合接合」專利；⭐⭐⭐ 雙材料 CMP 的兩條相反路線）
  - `wiki/technologies/glass-substrate.md`（⭐⭐⭐ 兩大失效模式在界面與孔緣；⭐⭐⭐ 剖面五分類；⭐⭐⭐ Lau 兩難的第一個業界回應是「繞過」）
  - `wiki/technologies/foplp.md`（⭐⭐⭐ 成本模型下修至 310×310；⭐⭐⭐ 翹曲峰值在 debonding、承載板為新變數；⭐⭐ Lam ~100×100 mm 門檻）
  - `wiki/technologies/tsv.md`（以放寬 AR 換製程寬容度；模封銅孔為第三種垂直互連路徑）
  - `wiki/concepts/thermal-management.md`（⭐⭐ 「熱」拆為運作熱 vs 製程熱；製程熱新增第三個切入點）
  - `wiki/concepts/test-metrology-packaging.md`（⭐⭐⭐ 「粗糙度」跨域落差 2–4 個數量級；形態學分類是量測規範的上游）
  - `wiki/entities/applied-materials.md`（✅ 產品層事實由官網一手確認；❌ 市占仍未結清；⭐⭐ 以 liner 切入 TGV）
  - `wiki/entities/intel.md`（⭐⭐ 排他權下在 TCB 鍵合頭噴嘴幾何；雙 family 圍籬）
  - `wiki/entities/corning.md`（⭐⭐⭐ 獨立第三方確認其所賭的界面失效模式為真）
  - `wiki/entities/tsmc.md`（雙材料同時 CMP 的排他權）
  - `wiki/index.md`（頁面總數 589→604；來源數 536→550）
  - `wiki/overview.md`（2 項結清、4 項提問方式修正／降級、新增 9 項空缺與 12 條橫向論述）
- 主要新知識：
  1. ⭐⭐⭐ **Cu dishing 是雙向失效參數，而本 wiki 此前只記錄了單向。** Cu–Cu 綜述（大連交通大學，*J. Sci. Adv. Mater. Devices* 11(2) 101155，2026-03-27）：**dishing 不足 → 空洞；dishing 過度 → 間隙無法閉合**；控制能力 **3–5 nm**；NineScrolls 另稱**容許窗口常僅約 2 nm 寬**。➜ 兩個後果：（a）**「關鍵參數不是單調的」增至第七例，且首次有上下界皆明確的物理機制**，論述可收斂為「**當一個參數同時服務兩個相反的失效模式，最佳值必然是區間而非極值**」；（b）**2026-09-21 列為最高優先的「Cu recess 產線實績」空缺降級**——2023-09 的 5–25 nm 產線實績與本篇的 3–5 nm 控制能力落差 **2–5 倍**，原因果鏈成立但須改述為「**已可達標、尚未穩定**」，追蹤方式改為「找出一個明確標註為產線的 dishing 分布」。附帶：**SiCN <2 Å RMS 為第三個獨立來源，該值自「已交叉確認」升為產業共識值**；**接合能首次有介電層排序**（SiCN/SiCO 2.32 vs SiON/SiON 1.5 J/m²，差 ~1.5×），解釋了為何 SiCN 成為預設且 AMAT 為此設有 Insepra™ 專屬平台。
  2. ⭐⭐⭐ **同一輪內首次同時取得一個瓶頸的「代價量級」與「規避行為」兩種證據。** 代價側：NineScrolls **「1 µm 顆粒可誘發數百微米寬的空洞」**（放大 10²–10³ 倍，本 wiki 的「潔淨度」首次脫離形容詞）。規避側：**珠海天成 CN121666157A** 是本 wiki 第一件把**「避免了對前道混合鍵合設備與超高潔淨環境的依賴」**寫成明示技術效果的專利（模封開孔 AR ≤10 + TSV 熱壓鍵合，目標晶片 InP）。➜ 「潔淨度／設備門檻是成本項」自本 wiki 推論**升格為有申請人以排他權下注的商業判斷**（⚠ 中國中型業者，非產業共識）。➜ 並歸納出**新橫向模式：當某製程規格難度陡升時，業界的第二條路不是改進該製程，而是把設計移到規格較鬆的區間**（另例：面板圖案化的粗快／細慢分工）。
  3. ⭐⭐⭐ **「CMP 是限制層」取得目前最強的間接佐證：兩種完全不同的商業模式在同一處下注，且方向相反。** **Adeia EP4709132A2**（2026-03-11）以阻障層作研磨停止層，**把一次研磨兩種材料拆成兩段**；**TSMC CN120035162A**（2025-05-23）以**二氧化鈦–二氧化矽混合顆粒磨料使兩種材料可同時研磨**。➜ 一家以界面 IP 授權為業、一家是全球最大晶圓代工廠，**兩者都認為「雙材料同時 CMP」值得持有排他權**。⚠ TSMC 該件請求項未提及混合接合，僅支持較弱陳述。附帶趨勢：**磨料自單一氧化物轉向含鈦混合顆粒**（另見 YCCHEM WO2026147117A1，純配方未選入）。
  4. ⭐⭐⭐ **TGV 的失效在界面與孔緣，不在材料本體；且本 wiki 首次能把 TGV 的蝕刻端與可靠度端連成一條因果鏈。** **AMAT（德國）IMAPS DPC 2026**：兩種主導失效為**① 種子層附著不足 → 銅剝離；② 孔緣應力集中 → 玻璃開裂**，解法為**多層 liner（模數 + CTE 雙參數最佳化）**。**Micromachines 綜述（2026-09-20）**：側壁粗糙度、垂直度、內部殘留「對種子層覆蓋、銅填充完整性、高頻損耗與熱機械可靠性有深遠影響」。➜ 完整鏈：**側壁形態 → 種子層覆蓋 →（AMAT）附著不足 → 銅剝離**。➜ 且 **Corning WO2026164778A1 正是針對模式 ① 的解**——本 wiki 2026-09-18 記的「Corning 賭界面可做牢 vs Intel 賭界面必失效」，至此由獨立第三方**確認賭注標的為真**（AMAT 未裁定勝負，其 liner 是第三條路線）。➜ **玻璃基板頁的論述重心應自「材料選擇」移向「界面工程與孔緣幾何」。**
  5. ⭐⭐⭐ **TGV 剖面有五種形態，其中沙漏形的「腰部高度」本身是獨立變數。** （直壁／沙漏／等腰錐／倒錐／底切；Borofloat 33 於 49% HF 錐角 **45°–50°**；LIWE 側壁粗糙度 **1.257 µm → 25 nm**，~50×；陣列孔徑 **RSD <1%**，⚠ 未附重複性。）➜ Onto（2025-10-06）給的是「頂／腰／底三個 CD」的**量測要求**，本篇給的是**物理成因**。**列管空缺「Corning small via diameter」提問方式第三次修正**。➜ 另：**「粗糙度」一詞的跨技術域落差首次量化**——TGV 側壁 25 nm–1.257 µm vs 混合接合 Ra <0.1–0.2 nm，**相差 2–4 個數量級**，跨頁引用必須標註技術域。
  6. ⭐⭐⭐ **面板論述本輪同時向兩個方向收斂：尺寸收斂到 310，適用範圍收斂到大封裝。** **Amy Lujan（IMAPS DPC 2026）把成本分析自 600×600 下修至 310×310 mm**，並限定於「大型且複雜的封裝 + advanced fan-out」；**Lam（一手，2026-06-25）**給出**「封裝超過約 100×100 mm 後晶圓即失去效率」**這個本 wiki 首個設備商幾何門檻，並確認**面板尺寸標準業界未定**。➜ 「面板三陣營」改記為：**310 陣營正取得成本模型側支持；510×515 與 600×600 陣營至今對吞吐量問題無任何公開回應**。⚠⚠ **Lujan 的淨成本結論未取得（僅摘要），在吞吐劣勢與良率劣勢皆已量化後，這是面板論述最關鍵的單一未知數，列下輪最高優先。** ⚠ 另：**Lam 的 ~100×100 mm 門檻與 CoWoS 14× 光罩（~1,180 mm²）相差近一個數量級**，列為新空缺。
  7. ⭐⭐⭐ **「真正的瓶頸在被視為輔助步驟的那一步」在本輪同時出現兩個跨技術域實例。** 混合接合側：NineScrolls 斷言**「CMP 後清洗是繼 CMP 本身之後最大的單一良率槓桿」**。面板側：成功大學×南臺科大（*Sci. Rep.*，2026-06-15）FEA 顯示 **molding-first 的翹曲峰值出現在 debonding 階段**，且**承載板材料（鋼／玻璃／陶瓷）是本 wiki 此前完全沒有記錄的變數**。⚠ 兩者皆單一來源、皆未附完整數據，列為**橫向論述候選**，既有的限制鏈排序與翹曲論述**均不逕行改寫**。
  8. ⭐⭐ **「熱」必須拆成運作熱與製程熱兩條線。** **Intel US20260191069A1 + EP4770413A2**（2026-07，**兩個不同 family、不同發明團隊、相鄰日，各主張一種幾何**）把**熱傳耦合器（TCB 鍵合頭噴嘴）的內部流道幾何**寫成請求項。➜ 本 wiki 既有的熱記錄幾乎全屬**運作熱**；**製程熱**此前僅有 775 µm 熱預算與退火溫度兩個切入點，本輪新增第三個：**鍵合頭本身**。⭐ 這也是「圍籬式布局」的第三個實例，**首次出現在設備配件層級**。
  9. ⭐⭐ **論文是落後指標，不是領先指標。** **CEA + SET Corporation JP2026501395A**（2026-01-14 公開，**優先權 2022-12-28**）為 D2W 直接混合接合方法；對照 CEA-Leti 的 **ECTC 2026 D2W 1 µm** 發表，本 wiki 首次量到同一機構「**排他權布局 → 學術發表**」的間隔約 **3–4 年**。➜ **所有會議發表的解讀都應加上這個時間位移。** 附帶：D2W 設備側 IP 不只 Besi／ASMPT／EVG 三強，法國 SET 亦在列。
  10. ⭐⭐ **2026-09-21 新設的「官網產品頁複核」規則首次用於正向確認。** AMAT 官方混合接合頁明列 **Opta、Catalyst = CMP 平台**、**Insepra™ SiCN = 混合接合表面製備**、合作為 **EVG（W2W）／Besi（D2W）**。➜ ✅ **產品層事實結清**（2026-09-21 只否證了「Lam SABRE 3D = 混合接合 CMP」，並未確認 AMAT 自己有無）；❌ **「市占 100%」仍無任何佐證，維持待證且不得作為其他推論前提**。⚠ 值得記錄的是：**一家把 CMP 列入產品線的公司，在公開頁面把挑戰列為「無缺陷／對準誤差近零／成本更低」三項，完全不提表面平坦度**——兩種讀法（已解決／不願強調），本 wiki 不裁定。
  11. ⭐⭐ **TC bonding 與 hybrid bonding 不是同一條學習曲線。** Hanmi 在 TC bonder 領先（SK hynix ₩442 億訂單、HBF 軌首批交付），但其混合接合機**第一代 2020 → 第二代原型 2026 年底 → 廠房 2027 上半（₩100B／Class 100）→ 量產採用 ~2029**。➜ 若 HB 確於 2027 年底進入 HBM4E，**Hanmi 將缺席整個世代**，該世代機台由 Besi／ASMPT／EVG／Hanwha 供應。⚠ Hanmi 未揭露任何對準精度或吞吐量，**公開具體度明顯落後 Besi Kinex（100 nm @ 3σ、1,600–2,000 die/hr）**；其 2026 年四件專利亦偏向機台工程層而非接合物理層，兩項觀察同向。
- 專利訊號：本輪五件分為四條主線。（1）⭐⭐⭐ **規避路線首次出現**——珠海天成以模封銅孔 + TCB 明示規避混合接合設備與超高潔淨環境（見上第 2 點）。（2）⭐⭐⭐ **CMP 的雙向下注**——Adeia 拆成兩段 vs TSMC 同時研磨（見上第 3 點）。（3）⭐⭐ **Intel 把熱的排他權下在鍵合頭上**，且以雙 family 圍籬（見上第 8 點）；⚠ 值得追問：其 EMIB-T 外包夥伴 Amkor 使用的是誰的鍵合頭。（4）⭐⭐⭐ **玻璃的用途限制策略**——上海美維（**PCB／載板業者**，本 wiki 第二型「邊界外擴」：載板業者向上游堆疊製程延伸）以 CTE 與矽匹配的玻璃為**堆疊載板**，循環執行「TCB → 介質包覆 → CMP 減薄 → 層間互連」，外部互連走背面。➜ 這**落在 Lau 兩難的贏的那一側而完全未處理 PCB 側劣勢**——**玻璃的 CTE 兩難可以用「限制用途」規避，代價是放棄取代有機載板的野心**。⭐⭐ 另：**CMP 被放在每一層之間 ➜ CMP 次數隨堆疊層數線性成長**，與「dishing 窗口僅約 2 nm 寬」合看，**每多一層就多一次落在窄窗口內的要求**——這是「CMP 是限制層」的新面向：限制不只在單次規格難度，也在次數。⚠ **本輪五件中僅 TSMC 一件有具體材料組成，四件無任何量化數值；「專利軌訊號以定性為主」連續第二輪成立。** 此外本輪檢出但未選入者值得記錄：**SanDisk 於 2026-05-28 同日以相同標題公開至少四個 family 的 D2W 記憶體結構案**（99884234 已於 2026-09-18 收錄，另三件 99884269／99884271／99889249 摘要近乎相同，依「重複家族」規則棄收）——**其圍籬規模比本 wiki 既記的「四個 family」更大**，已在 [[entities/sandisk]] 之外先記於此。
- 下輪建議：
  1. ⭐ **最高優先：取得 Lujan「Cost Analysis of Panel-level Packaging for HPC and AI Applications」（10.4071/001c.167018）全文。** IMAPSource 為 OA 平台（2026-09-21 已由 imapsjmep.org 成功取得 Lau 全文 PDF），本輪 `imapsource.org/article/167018-...` 僅回傳摘要頁，應改試其 PDF 路徑。
  2. ⭐ **次高優先：Wiley Advanced Science（Kang, 10.1002/advs.202512611）連續兩輪 403。** Advanced Science 為 OA，應改由 PMC／作者機構典藏／ResearchGate 以外之合法 OA 管道嘗試；若第三輪仍失敗，**降級並改以其引用文獻迂迴取得**。
  3. ⭐ **新增待追：AMAT IMAPS DPC 2026 的 TGV liner 全文**（10.4071/001c.167752）——需模數／CTE 數值與「有／無 liner」熱循環對照。與第 1 項同平台，可同批處理。
  4. **本輪 IMAPS DPC 2026 另檢出三篇未選入但高相關者，下輪優先**：`10.4071/001c.167746`（Fraunhofer IZM，Glass Core Substrates for High-Density Chiplet Integration）、`10.4071/001c.167027`（Tokyo Electron，Reliability Evaluation for Realizing Panel-Level Packaging）、`10.4071/001c.167502`（ASE，Embedded Bridge Interposers with Adaptive Patterning）。**IMAPSource 2026 DPC 整輯是本 wiki 尚未系統掃過的一手機構來源池。**
  5. **專利軌輪替**：本輪完成珠海天成、Intel（設備配件）、上海美維、TSMC（漿料）、CEA+SET。**未輪替者延續**：Besi、KLA／Bruker（量測軌）、Ibiden、Entegris／Fujimi／Resonac（本輪以 `ti,ab="polishing slurry"` 檢出 279 件但**漿料商的封裝架構內容仍為零**——⚠ 建議**放棄以漿料商為申請人檢索**，改以「使用端 + 製程詞」檢索，本輪 TSMC 該件即由此命中）。**EV Group 與 Hanmi 本輪以技術詞收斂後仍未命中封裝架構內容**，⚠ 兩者的 2026 年案件確實偏機台工程層，**建議自「待輪替」名單移至「已確認訊號偏弱」**。
  6. ⭐ **論文軌應鎖定「FOPLP 承載板材料」與「debonding 階段翹曲」兩個本輪新開主題**，兩者皆為單一來源且皆是本輪最強的待證項。
  7. 📌 **作業規則沿用確認**：本輪所有 OpenAlex 查詢皆經同一支去重腳本（`oa_filter.py`：正規化標題去重 → DOI 比對 `_collected_urls.txt` → 半導體關鍵詞 ≥2 命中過濾），無臨機人工挑選。**2026-09-21 記錄的規則有效。**
  8. 📌 **2026-09-21 所指出的去重結構性弱點仍未修復**：`_collected_urls.txt` 第 4 欄仍是人工摘要而非正規化原標題，**論文軌的去重實質上仍只有 DOI 一層防護**。本輪未觸發該弱點（無 Zenodo／預印本命中被選入），但**建議仍然有效：增設第 5 欄或另建 `raw/_titles.tsv`**。
  9. 既有未結案項延續：`10.1016/j.mssp.2026.111165` 全文、Nature Electronics CPO 綜述全文、CoWoS「5.5× 良率 99%」量測邊界、Samsung US20260247940A1 目標產品線、JCET 韓國團隊產能佐證、Silicon Box 第三方驗證、PHB 的量化數據、16-Hi HBM4 對賭的驗證、AMAT 混合接合 CMP **市占**（產品層已結清，市占未結清）。
...
## [2026-09-23] collect | 每日自動蒐集 — 先進封裝最新動態（三軌執行：新聞 + 專利 + 論文）
- 搜尋查詢數：新聞 5 組（WebSearch，全部有效命中）/ 專利 8 組（CQL；**全部 HTTP 200，無 404、無 403/429**）/ 論文 4 組（OpenAlex filter，全部 200）＋ IMAPS 全文直取 5 篇
- 成功抓取：**16 篇**（articles 5, patents 5, papers 6, reports 0）——其中 **papers 含 2 篇為既有摘要條目的全文升級**（`10.4071/001c.167018`、`10.4071/001c.167752`）
- 失敗/跳過：約 90 篇（**付費牆／內容保留 1**：PhotonCap「7 Bonding Equipment Companies」核心內容在付費段，僅取得可由 TheElec 交叉確認之數字，故棄收；**已收錄 5**：Tom's Hardware 玻璃基板路線圖、TrendForce 2026/04/13 CoPoS、atlaspcb CoPoS、MDPI `ma19163500`（FOPLP debonding ML 翹曲，2026-09-22 已收）、TrendForce 2026/07/22；**主動棄收（無封裝架構內容）約 10**：OPS 檢出之 Kulicke & Soffa 打線系列 15 件、Shinko 通用 wiring substrate 6 件、Amkor 標題同質之 "ELECTRONIC DEVICES" 系列；**低相關約 74**：OpenAlex 四組 100 筆去重後多為牙科 debonding、洋麻纖維複材、單寧樹脂、CFRP 加勁板等非半導體命中）
- 資料源狀態：WebSearch ACTIVE ✓ | EPO OPS ACTIVE ✓ | OpenAlex ACTIVE ✓（**OPS 9 次呼叫／上限 10**：1 auth + 8 search，**無 detail 呼叫**——本輪摘要皆自 search/biblio 回應直接取得，未另耗配額；無 403/429。OpenAlex 4 組全 200，無 429）
- 新增 wiki 頁面：**17 頁**（來源摘要頁 16 + 實體頁 1）
  - `wiki/entities/hanwha-semitech.md`（⭐ 結清列管之實體頁空缺；觸發點：取得 SHB2 Nano 的對準數值與時程）
- 更新 wiki 頁面：**13 頁**
  - `wiki/technologies/hybrid-bonding.md`（⭐⭐⭐ D2W 僅占市場 4.5% 的敘事重心修正；⭐⭐⭐ 限制鏈首次取得物理機制；⭐⭐⭐ 設備側時程收斂 2029–2030；⭐⭐⭐ Besi inline 品質判定只適用 TCB）
  - `wiki/technologies/foplp.md`（⭐⭐⭐ Lujan 0.5 個百分點；⭐⭐⭐ 公差預算下移的量化實例；⭐⭐⭐ 載板材料不是自由變數；⭐⭐ 面板擴散至車用）
  - `wiki/technologies/glass-substrate.md`（⭐⭐⭐ 開裂兩階段機制；⭐⭐⭐ liner −60%／2×；⭐⭐⭐ Samsung EM 可靠度驗證失敗之矛盾；⭐⭐ Fraunhofer 逐年路線與 1/3 孔徑比）
  - `wiki/technologies/copos.md`（310×310 的成本基礎比此前假設脆弱）
  - `wiki/concepts/test-metrology-packaging.md`（⭐⭐ 量測往前推的三個獨立實例，明示不合併為單一趨勢）
  - `wiki/concepts/advanced-packaging-market.md`（⭐⭐⭐ 會計邊界檢核規則；混合接合市場分項）
  - `wiki/entities/besi.md`（⭐⭐⭐ 兩件專利；⚠ 對 Hanmi 推論的反例）
  - `wiki/entities/applied-materials.md`（TGV liner 全文；❌ 市占仍未結清）
  - `wiki/entities/samsung.md`（玻璃可靠度失敗 + P5 50 台產線 + Semes W2W）
  - `wiki/entities/ase-group.md`（Adaptive Patterning 公差表）
  - `wiki/entities/ev-group.md`（⚠ 輪替名單應按題材而非按公司）
  - `wiki/entities/hanmi.md`（⚠ 空缺提問方式修正 + 推論反例）
  - `wiki/entities/amkor.md`、`wiki/entities/asml.md`
  - `wiki/index.md`（頁面總數 604→621；來源數 550→566）
  - `wiki/overview.md`（6 項結清、4 項降級／提問方式修正、9 項新增空缺、14 條橫向論述）
- 主要新知識：
  1. ⭐⭐⭐ **面板論述的最高優先未知數結清，且結論偏負面。** **Lujan（SavanSys, IMAPS 22nd DPC 2026）全文**：310×310 mm 面板在大型複雜封裝上**有淨節省，但 0.5 個百分點的良率差距即完全抹平**（"With a 0.5% yield loss, the panel-based package is no longer cost-effective."）。本 wiki 已記玻璃面板良率 **70–85%** vs 有機 **>90%**——**該落差是容忍度的 10–40 倍**。➜ **在大型複雜封裝上，面板的成本優勢在現行良率水準下不成立，差距達一個數量級以上。** ⭐⭐⭐ 附帶更重要的方法論發現：**排除 GPU/HBM 晶粒成本時，容忍度變成 ~60%——同一份分析、兩種會計邊界，結論相差逾兩個數量級**，作者自承「切法不同可得出巨大或微小降本」。➜ **新增常駐檢核規則：本 wiki 收錄的任何「降本 N%」宣稱都必須追問其會計邊界。**
  2. ⭐⭐⭐ **限制鏈排序首次取得物理機制解釋。** Au–Au 表面活化直接接合綜述（*Sensors*，Binghamton × 東京大學）：**"heat and pressure in TCB can deform asperities and increase real contact area"** ➜ **TCB 自帶「就地整平」機制，混合接合沒有，因此表面必須在接觸之前就已合格。** ➜ 這解釋了本 wiki 的限制鏈為何以**表面平坦度**為第一限制——**它是唯一無法在接合當下被補償的項目**；也解釋了「1 µm 顆粒誘發數百微米空洞」。⚠ 材料為 Au（不氧化），**可安全遷移的是力學部分，非化學部分**。➜ 並使「TC bonding 與 hybrid bonding 不是同一條學習曲線」**自商業觀察升格為機制**，且本輪同時取得**第二家公司的同型證據**（Hanwha 的 TCB 為 ₩900 億級、HB 仍在測試，與 Hanmi 同型）與**第三個維度**（Besi 的 inline 品質判定只適用 TCB）。
  3. ⭐⭐⭐ **本 wiki 的混合接合敘事需要結構性修正：D2W 只占市場的 4.5%。** TheElec（2026-04-28）：混合接合市場 2025 年 **>US$6B**，**D2W 僅 US$275M（4.5%）**，**W2W 約 95%**。➜ **D2W 是「難而新」的那一塊，不是「大」的那一塊。** 本 wiki 此前的全部討論集中在 D2W，容易讓讀者把「混合接合的困難」誤讀為「混合接合尚未量產」。➜ 機台單價 ₩40–50 億，對照 High-NA EUV €3.5 億（**~100 倍**）。
  4. ⭐⭐⭐ **「把公差預算從上游移到下游」首次取得量化實例，且同一策略同年出現在製程鏈的兩個環節。** **ASE×Deca**（IMAPS DPC 2026）：埋入式橋接晶粒擺放公差自傳統的 **±3 µm** 放寬至 **±10–40 µm（3.3–13.3 倍）**，機制是 **Adaptive Patterning——逐顆量測位置後以客製化微影遷就晶粒**，且每顆獨立補償故**良率不隨橋接晶粒數惡化**。➜ 2026-09-22 的橫向論述第 3 條（原兩例皆定性）**精確化為：「當上游的位置精度成本陡升，下游若具備可程式化的圖案能力，最佳解是把公差預算從上游移到下游。」** 關鍵前提是**下游可程式化**——這也解釋了為何同一招**不能**用在混合接合。➜ ⭐⭐⭐ **Besi WO2026192456A1（可受控移動的定心銷）是同一策略的機構層版本**：Besi 讓治具遷就載具的尺寸漂移，Deca 讓微影遷就晶粒的位置漂移。**兩家公司、兩個環節、同一年。**
  5. ⭐⭐⭐ **TGV 玻璃開裂的完整機制取得，且起始與擴展發生在熱循環的相反半程。** AMAT 全文：**受熱時 Cu 膨脹形成三點壓應力 → 裂紋起始；冷卻時 Cu 收縮產生軸對稱拉伸力 → 環向裂紋擴展。** ➜ 這解釋了兩件本 wiki 此前只能描述的事：為何 TGV 失效以**熱循環次數**而非峰值溫度為主導變數；為何**環向裂紋**是特徵失效形貌。➜ **CVD liner 降應力至多 60%、Ti/Cu 種子層附著至多 2×、沉積溫度 80–400 °C**（結清「有／無 liner 對照值」空缺）。⚠ 皆為 AMAT 內部數據、**無絕對值**，故「TGV 陣列力學數值」空缺**不關閉**。➜ ⭐⭐⭐ **liner 的模數 × CTE 是二維互相拉扯的設計空間**（低模數有利緩衝玻璃應力，卻在回流後剝離）——**「最佳值必然是區間而非極值」的第八例，且是首個二維實例**（第七例 Cu dishing 為一維）。
  6. ⭐⭐⭐ **玻璃基板的產業瓶頸首次被定位到一個具體閘門，且與 AMAT 的機制指認一致。** BigGo（2026-08-12）：**Samsung EM 送樣未通過客戶端可靠度驗證**，量產 2H27→2028→**2028+**，設備採購自 2025-11 反覆延後、2027-04 到機計畫已放棄。⚠⚠ **與本 wiki 2026-08-23 記錄的「Samsung EM 否認認證失敗」直接衝突，且本篇早於該否認。本 wiki 不裁定**，但記錄：**時程的三次後退是雙方都不否認的客觀事實**；且 **AMAT 指認的兩大 TGV 失效（種子層附著、孔緣開裂）皆屬可靠度範疇，與 Samsung EM 卡關的環節同一層級**——兩個獨立來源、不同角度，指向同一處。➜ 附帶：**LG Innotek 改以人形機器人影像感測器切入**，是本 wiki 第三個「降低要求以先取得經驗」實例，**但首次放寬的不是製程規格而是應用領域的可靠度門檻**。
  7. ⭐⭐⭐ **混合接合設備側的三個獨立時間點收斂於 2029–2030，「HBM4E 導入」缺乏設備側佐證。** TheElec（2026-07-22）：Samsung 平澤 **P5 規劃 ~50 台 D2W 機台**，**首選 Besi（₩60 億／台 ≈ US$4.6M）**，安裝 2026 年底起、**大規模量產 2030**。➜ 與 **Hanmi 的 ~2029**、**SK hynix 的「HB 延後至 HBM5」**收斂。➜ ⭐⭐⭐ **資本量級首見**：50 × ₩60 億 ≈ **₩3,000 億（US$2.3B）僅機台**，對照 Hanmi 的**廠房 ₩1,000 億** ➜ **機台與廠房差一個數量級**，本 wiki 此前只記得到廠房側。➜ ⚠ **首見採購方反向驅動機台規格**（「Samsung 要求機台設計變更」），列為最值得追的新空缺。
  8. ⭐⭐⭐ **「Hanmi 是否缺席 HBM4E 世代」的供應者問題結清：Hanmi 可能缺席，韓系不缺席。** **Hanwha Semitech SHB2 Nano 對準 0.1 µm、2026 H1 進入客戶測試**（第一代 2022-01），**比 Hanmi 的「2026 年底原型」早約一年**；另有 Besi（Samsung P5 首選）與 **Semes（Samsung 自製，走 W2W）**。⚠ 0.1 µm **未標註是否 3σ**，屬規格宣稱，不可與 Besi Kinex 的 100 nm @ 3σ 量產實績等同。
  9. ⭐⭐ **「把量測往前推」同年出現在三個不同位置。** 接合端：**Besi WO2026182734A1** 以**液相焊料表面張力在接合當下判定品質、無需破壞性測試**（本 wiki 第一件把量測移進接合動作本身的專利）；測試端：**Amkor US20260202438A1** 同軸纜線貫穿測試壓件（**待測封裝頻率已高到測試接點本身成為誤差源**）；量測端：**KLA CN121488155A**（本 wiki 首件 KLA 專利）**以取樣次數換訊噪比**，呼應 2026-09-21 的「TSV 深度量測重複性 2.18 µm ≈ 陣列分散 2.15 µm」。⚠⚠ **三者無任何引用關係，僅為同向觀察，明示不得合併敘述為單一趨勢。** ➜ 附帶：Besi 該件**只適用 TCB**（HB 無液相焊料），➜ **TCB 側已有 inline 非破壞品質判定，HB 側本 wiki 一件也沒有**。
  10. ⭐⭐ **載板材料不是自由變數。** **EVG TW202611995A**：雷射可溶層解接合**要求連接層與載板對該輻射透明** ➜ 推得因果鏈：**解接合方式 → 載板須透明 → 排除多數金屬載板 → 載板材料被解接合製程反向決定 → 而載板材料又決定翹曲。** ⚠ 此為本 wiki 推論（EVG 專利 × 2026-09-22 成大 FEA 的交叉），非任一來源明述。
  11. ⭐⭐ **Fraunhofer IZM 給出玻璃核心基板的逐年量化路線**：TGV **≤50 → ≤30 µm**、RDL **SAP 5 → 2 µm L/S**（研究中 damascene 1 µm）、via **60 → 15 → 5 → 1 µm**。⭐ **同厚度（≤500 µm）下玻璃 TGV 50 µm vs 有機/玻璃織物 150 µm ➜ 孔徑為 1/3**（本 wiki 首次把玻璃密度優勢化為單一比值）。**PVD 受深寬比限制，改用底部向上電鍍**，與 2026-09-20 的「真空輔助無孔洞 Cu 填充 AR10」指向同一瓶頸的兩種解。⭐ **TGV 金屬化鏈末端是 CMP** ➜ **「CMP 是限制層」的適用範圍自混合接合擴及玻璃基板**。⭐ **lint 待辦取得可操作依據**：GCS 走 SAP/ABF/PID、中介層走 damascene ➜ **拆頁應依製程鏈而非依用途**。
  12. ⭐⭐ **面板向第二個應用域擴散：車用功率模組**（AOI Electronics）。shadow moiré 測得**回流溫度下翹曲變化很小**，與 2026-09-22 的成大 FEA **方向一致**——共同支持「**回流不是翹曲峰值**」這半句，但 AOI **未量測 debonding 階段**，故該空缺**不結清**。⚠ **申請人更正**：2026-09-22 把 `10.4071/001c.167027` 記為 **Tokyo Electron**，**全文確認為 AOI Electronics（日本香川）**。
- 專利訊號：本輪五件分為三條主線。（1）⭐⭐⭐ **量測往前推**——Besi 把接合機變成量測儀（僅 TCB）、Amkor 把同軸做進測試壓件、KLA 以取樣次數換訊噪比；⚠ 三者無關聯，不合併。（2）⭐⭐⭐ **公差預算下移**——Besi 可移動定心銷（機構層）與 ASE/Deca Adaptive Patterning（微影層）構成同一策略的兩個環節。（3）⭐⭐ **載板的隱藏約束**——EVG 的透明性要求使載板材料被解接合製程反向決定。⚠ **本輪五件全部無任何量化數值，「專利軌訊號以定性為主」連續第三輪成立**；本輪最強的量化全部來自論文軌（IMAPS DPC 2026 全文）。⚠ **對 2026-09-22 一項推論的反例**：該輪記「Hanmi 專利偏機台工程層 ⇒ 訊號偏弱」，但 **Besi 2026 年兩件同樣偏機台工程層，而其量產實績遠領先** ➜ **「專利偏機台工程層」不可推論為「技術落後」**，原記載的兩項觀察應拆開（「公開具體度落後」仍成立）。
- 作業面重大改善（三項長期待辦一次結清）：
  1. ⭐⭐⭐ **IMAPSource 全文取得路徑確認並驗證**：**`https://imapsource.org/article/<id>.pdf`**（**非** 2026-09-22 試過的摘要頁，**非** 2026-09-21 對 Lau 有效的 `imapsjmep.org`）。**本輪對 5 篇全部驗證成功**（167018、167752、167746、167027、167502）。➜ **IMAPS DPC 2026 整輯自此可系統掃描**，2026-09-22 列為「本 wiki 尚未系統掃過的一手機構來源池」者，取得路徑已解決。
  2. ⭐⭐⭐ **去重結構性弱點修復（連續三輪建議，本輪落實）**：新建 **`raw/_titles.tsv`**（4 欄：`normalized_title` / `original_title` / `identifier` / `path`），自全部 **576 個 raw 檔的 frontmatter 自動抽取**（僅 1 檔缺 title）。➜ **標題去重自此真正可用**，不再只有 DOI 一層防護。⚠⚠ **建立當下即檢出 18 組正規化標題重複**（如 "making hybrid bonding better" ×2、"packaging technologies redefine ai and hpc scalability limits at ectc 2026" **×3**、imec/EVG W2W 200nm ×2 等）——**這正是 2026-09-21／22 兩輪所警告而 DOI 比對抓不到的那一類重複，且數量比預期多**。**本輪未刪除任何 raw 檔**（依 spec「raw/ 僅 `_collected_urls.txt` 可附加」），清單留待人工處置。
  3. ⭐⭐ **兩項 lint 待辦結清**：
     - `wiki/technologies/info-wmcm.md` **非合法 UTF-8**（自 2026-09-15 列為最優先）——**已修復**：原檔在第 4,721 位元組截斷於不完整的多位元組字元中間，本輪移除該殘餘位元組序列並加註修復記錄。⚠ **被截斷的句子內容未以任何方式補寫或臆測**，需自 raw 來源重建，列後續待辦。**repo 全域複查：`wiki/**` 與 `raw/**` 共 0 個非法 UTF-8 檔。**
     - `wiki/index.md` **來源清單全域重排**（2026-09-22 記為 70 組非遞減相鄰對）——**已完成**：553 筆來源條目原地重排，**非遞減相鄰對 72 → 0**；以 `diff` 驗證排序前後**內容逐行完全相同，僅順序改變**（唯一無完整日期者 `2026-05-xx` 以當月 15 日為排序鍵，原文未改寫）。
- 下輪建議：
  1. ⭐ **最高優先：系統掃描 IMAPS DPC 2026 整輯。** 取得路徑已驗證（`imapsource.org/article/<id>.pdf`），本輪 5 篇皆為高價值。建議以 OpenAlex 檢索 `10.4071/001c.*` 於 2026 年之全部條目，建立 id 清單後分批取全文。**這是目前投入產出比最高的來源池。**
  2. ⭐ **次高優先：處理 `raw/_titles.tsv` 檢出的 18 組重複。** 需人工裁定保留哪一份（或建合併轉向頁，wiki 既有 8 個先例）。**在處置前，這 18 組會使 `index.md` 的來源計數略為高估。**
  3. ⭐ **Wiley Advanced Science（Kang, `10.1002/advs.202512611`）已連續兩輪 403，本輪未再嘗試**（依 2026-09-22 之「第三輪仍失敗則降級」規則，本輪主動把配額投入 IMAPS 全文，事後證明正確）。➜ **建議正式降級**，改以其引用文獻迂迴取得。
  4. ⭐ **追 Samsung 要求 Besi 做的機台設計變更內容** —— 若涉對準或吞吐，是本 wiki 目前最直接的「量產方認為機台還差什麼」訊號。
  5. **專利軌輪替**：本輪完成 **Besi ×2、EV Group、KLA、Amkor**。**未輪替者延續**：**Bruker**（量測軌，兩輪未動）、**Ibiden**（本輪 `pa="ibiden" or pa="shinko electric"` 僅命中 6 件且全為 Shinko 的通用 wiring substrate，**Ibiden 零命中**）、**Micron**、**Semes**（本輪 `pa="hanwha" or pa="semes"` 命中 5 件**全為 Hanwha 太陽能／化學，無 Hanwha Semitech**）。⚠ **新發現：Hanwha Semitech 在 EPO 無 2026 年接合相關公開**，推測為 KR 優先案尚未進入 EP/PCT 階段——**與其「2026 H1 才進客戶測試」的時程相符**，不應解讀為訊號偏弱。⚠ **Amkor 以標題檢索幾乎無效**（85 件中絕大多數標題同為 "ELECTRONIC DEVICES AND METHODS OF MANUFACTURING"），後續應以技術詞收斂。
  6. ⭐ **論文軌應鎖定「FOPLP debonding 階段的直接翹曲量測」** —— 本輪 AOI 只覆蓋了「回流不是峰值」這半句，debonding 階段仍只有單一 FEA 來源。
  7. 📌 **OpenAlex 本輪訊號明顯偏弱**（4 組 100 筆，去重後 20 筆候選中僅 3 筆真正相關，其餘為牙科 debonding、洋麻纖維、單寧樹脂等）。⚠ **「debonding」與「warpage」是高噪音詞**，應與半導體限定詞強制共現，或改以 `10.4071/001c.*`／venue 導向檢索。**本輪的高價值全部來自 IMAPS 直取而非 OpenAlex 關鍵詞檢索**，此一觀察應影響下輪的配額分配。
  8. 📌 **作業規則沿用確認**：本輪所有 OpenAlex 查詢皆經同一支去重腳本（`oa_filter.py`：正規化標題去重 → DOI 比對 → 半導體關鍵詞 ≥2 命中過濾），無臨機人工挑選。**且本輪起 `raw/_titles.tsv` 已可供標題比對使用（本輪為建立輪次，尚未用於過濾；下輪起應併入 `oa_filter.py`）。**
  9. 既有未結案項延續：`10.1016/j.mssp.2026.111165` 全文、Nature Electronics CPO 綜述全文、CoWoS「5.5× 良率 99%」量測邊界、Samsung US20260247940A1 目標產品線、JCET 韓國團隊產能佐證、Silicon Box 第三方驗證、PHB 量化數據、16-Hi HBM4 對賭驗證、AMAT 混合接合 CMP **市占**（產品層已結清）、**Amkor 使用誰的 TCB 鍵合頭**、`info-wmcm.md` 被截斷句子的內容重建。
...
## [2026-09-24] collect | 每日自動蒐集 — 先進封裝最新動態
- 搜尋查詢數：新聞 **10 組** / 專利 **4 組 CQL** / 論文 **3 組**（① IMAPSource Proceedings 場館全掃 ② 面板/暫時接合片語組 ③ 混合接合/CMP 片語組）
- 成功抓取：**15 篇**（articles 4, patents 5, papers 6, reports 0）
- 失敗/跳過：約 **95 篇**（**取用失敗 1**：Digitimes `a20260904PD233`「SoIC vs CoWoS / HBM5 HB / EMIB-T 良率」**403**，依 spec 不重試——所幸 EMIB-T 良率一項由 ChainCatcher 另路取得；**已收錄 12**：TrendForce 2026/08/18 Shinko 22 層、EDN CPO、MLQ CPO 四路線、Tom's Hardware 玻璃路線圖、`10.3390/ma19163500`、`10.1038/s41598-026-47275-2`、insights.trendforce 混合接合、Tom's「EMIB-T heads for fab rollout」、SemiEng「flat-enough warpage」「advanced-packaging-limits」、3DInCites「engineering copper grain structure」「IFTLE 648」；**主動棄收（低訊號）約 30**：Atomica「AI Thermal Microstructures Platform」**全文無任何技術數據**（依品質過濾第一條棄收）、OPS 檢出之 Bruker 生命科學/NMR/質譜案約 20 件、Semes 通用 substrate-treating apparatus 6 件；**低相關約 52**：OpenAlex 片語組去重後多為牙科 direct-bonding、鈣鈦礦、柏油路面光纖感測、SiC CMP 等非封裝命中）
- 資料源狀態：WebSearch ACTIVE ✓ | EPO OPS ACTIVE ✓ | OpenAlex ACTIVE ✓（**OPS 6 次呼叫／上限 10**：1 auth + 4 search + **1 detail**（用於確認 JP2026119036A 申請人，epodoc 欄位為空、以 original 欄位確認為 Intel）；無 403/429。OpenAlex 全 200，無 429）
- 新增 wiki 頁面：**14 頁**（全部為來源摘要頁；**無新建實體/技術頁**——本輪 5 個新申請人〔Plan Optik、CIT、Okuno、Dongwoo、Atotech〕皆僅單件/單篇入庫，未達建頁門檻，改列 overview 缺頁清單）
  - 註：15 個 raw 檔 → 14 頁，因 **Intel 兩件 liner 專利合併為一頁**（`2026-09-24_patent_intel-tgv-liner-three-axis-fence`），兩者構成同一布局
- 更新 wiki 頁面：**12 頁**
  - `wiki/technologies/hybrid-bonding.md`（⭐⭐⭐ W2W 140 nm；⭐⭐⭐ 限制鏈標註「僅適用 D2W」；⭐⭐⭐ Q-time 雙機制；⭐⭐⭐ 三種 ECD 銅無一全勝；⭐⭐ 顆粒污染第二來源）
  - `wiki/technologies/glass-substrate.md`（⭐⭐⭐ 瓶頸在金屬化；⭐⭐⭐ 三條互斥 PVD 替代路線；⭐⭐⭐ 因果鏈新首環；⭐⭐⭐ Intel liner 三軸圍籬；⭐⭐⭐ 玻璃未消除翹曲；BOE + 成本溢價 + 良率衝突）
  - `wiki/technologies/foplp.md`（⭐⭐⭐ 載板重複使用劣化；⭐⭐⭐ 220 mJ/cm² 閾值；⭐⭐⭐ 清潔負擔落點；⭐⭐⭐ 透明性逐層傳播）
  - `wiki/technologies/emib.md`（⭐⭐⭐ 首個良率數字 50%；⭐⭐⭐ 獲利保證機制）
  - `wiki/technologies/tsv.md`（⭐⭐⭐ AR≈10 為跨基材的金屬化天花板；⭐⭐ 孔電阻為新驗收指標）
  - `wiki/concepts/thermal-management.md`（⭐⭐⭐ D2C/D2P/散熱片三分法；⭐⭐⭐ 冷卻與封裝競爭面積）
  - `wiki/concepts/advanced-packaging-market.md`（⭐⭐⭐ 成本溢價 30–50%；⭐⭐⭐ 載板攤提為第一個有量測支撐的邊界項；⭐⭐⭐ 財務性風險轉移）
  - `wiki/concepts/test-metrology-packaging.md`（粗糙度域差第 N 例；孔電阻；Q-time 雙機制；TEL 為「合格揭露格式」正面範例）
  - `wiki/entities/intel.md`（EMIB-T 良率 + liner 六件布局表 + 玻璃 45 µm）
  - `wiki/entities/tel.md`（⭐⭐⭐ 首次取得 TEL 接合設備量化能力）
  - `wiki/entities/micron.md`（輪替完成 + ⚠⚠ 檢索式失效記錄）
  - `wiki/entities/corning.md`、`wiki/entities/ev-group.md`
  - `wiki/index.md`（頁面總數 621→**635**；來源數 566→**580**；來源清單插入 14 筆後全域重排，非遞減相鄰對 **0**）
  - `wiki/overview.md`（6 項結清/降級/提問方式修正、11 項新增空缺、**18 條新增或修正之橫向論述**）
  - `raw/_titles.tsv`（+15 筆；**新增項未產生任何新的正規化標題重複**；既有 18 組重複仍待人工處置）
- 主要新知識：
  1. ⭐⭐⭐ **「混合接合」不是一條學習曲線，是兩條，且本 wiki 此前把 D2W 的數字寫成了整體狀態。** **TEL（IMAPS 22nd DPC 2026）在 140 nm 間距達成無孔洞 W2W 混合接合**，退火後 TEM 顯示**銅晶粒跨越接合界面成長**；最佳化配方後冠軍晶圓殘差 **<50 nm**、4 片中 **75% 殘差 <30 nm**、三片熔融接合晶圓 **99.5% 點位 <40 nm**、新硬體方案模擬可至 **14.6 nm M+3σ**。對照 D2W 量產 **6–9 µm** ➜ **W2W 領先 43–64 倍**。➜ **限制鏈（①表面平坦度 ~0.2 nm > ②die 翹曲 <100 nm > ③機台對準 100 nm）自此標註「僅適用 D2W」**——W2W 無第②層（整片晶圓），第③層已達 <50 nm 實測。➜ 與 2026-09-23 之市場結構（W2W ≈95% / D2W 4.5%）互相印證：**D2W 既不是大的、也不是領先的那一塊。** ⚠⚠ **載具僅有接合墊、無電性測試**（作者列 next step），故「140 nm 已成」目前只成立於「接得起來」層級。⭐ 附帶第一個**設計側**錯位來源：**Y 方向錯位較高，歸因於六角形接合墊佈局** ➜ **版圖對稱性本身進入對準預算。**
  2. ⭐⭐⭐ **能鑽出的孔，比能鍍滿的孔深一倍以上——TGV 的瓶頸在金屬化，不在成孔。** 成孔端：深紫外雷射 **AR 20:1–25:1**、LIDE 孔徑 3 µm（SemiEng 2025-09）。金屬化端：**濺鍍在 AR≈10 即難以成膜**（Okuno 明文，φ80 µm × 0.8 mm）、濕製程 + PPR/DC **AR 10–11 無孔洞**。➜ **本 wiki 的 TGV 論述重心自「成孔技術」移向「金屬化」**；且 **AR≈10 與 2026-09-20 之「真空輔助無孔洞 Cu 填充 AR10」同一天花板 ➜ 跨越玻璃與矽兩種基材。** ➜ ⭐⭐⭐ **同一堵牆、三條互斥解法同年到齊**：①底部向上電鍍（放棄側壁種子層；Fraunhofer / Intel US20260191064A1）②全濕式種子層（放棄真空製程；Okuno ZnO，**唯一附量產級實績者**）③ASE 改良濺鍍（不放棄 PVD；CIT EP4800151A1）。**本 wiki 首次能對單一瓶頸列出三條完整且互不相容的產業解法——這本身是「該瓶頸真實且尚無共識解」的最強指標。**
  3. ⭐⭐⭐ **Q-time 是兩個獨立物理機制共用的同一工程量，且本 wiki 先前只知其中一個。** 2026-09-22 已把「惰性環境 Cu 氧化」的提問方式修正為「實務形式是 queue time」。**Atotech（IMAPS DPC 2026）揭示第二個機制：體材再結晶**——可操作變數不是環境而是**電鍍電流密度與摻雜**：**0.5 ASD Q-time >3 週 vs 2.0 ASD <2 小時（>250 倍擺盪）**；fg-Cu 明文要求 **>1 個月**；DSC 顯示摻雜可把再結晶起始自 ~65 °C 推到 ~120 °C。➜ **兩機制必須分開記載。** ➜ ⭐⭐⭐ **且微縮方向與貯存窗口方向相反**：**孔徑越小、孔越深 ⇒ 再結晶越快 ⇒ Q-time 越短** ➜ **深寬比同時壓迫「鍍得進去」與「鍍完之後撐得住」兩端。**
  4. ⭐⭐⭐ **「最佳值必然是區間而非極值」取得第一個離散（材料選擇）版本，且新增一個 CMP 的作用方向。** Atotech 的三種 ECD 銅 × 四個驗收軸**無一全勝**：nt-Cu 贏低溫（<200 °C vs cg-Cu 的 >300 °C）但**孔填充不佳且 CMP 困難（粗糙度、硬度）**；cg-Cu 贏孔填充與 CMP 但**有界面孔洞風險**；fg-Cu 贏跨界面晶粒成長。➜ 論述擴充：**「當一個選項同時服務多個互斥驗收項時，最佳解可能不在任一極端選項，而必須被工程出來」**（此處為摻雜 fg-Cu）。前八例皆為連續參數。➜ ⭐⭐⭐ **「CMP 是限制層」取得全新作用方向：CMP 反向限制上游材料選擇**——不是「CMP 做不到」，而是「**因為 CMP 做不到，某些銅微結構不能用**」。與 2026-09-21 復旦 Ru nTSV「金屬硬到磨不動時 CMP 直接消失」為**同一機制的相反結局**。
  5. ⭐⭐⭐ **TGV 因果鏈的第一環可以是採購規格，而不在自家產線內。** **Plan Optik AG**（德，玻璃基板供應商）：**起始表面品質直接決定機械穩定性與製程良率——蝕刻會把原材的潛在微缺陷與次表面損傷曝露為應力集中點**（以硼矽/熔融石英各自的「標準拋光 vs MDF 進階拋光」對照佐證）。➜ 接上 2026-09-22 之四環鏈（側壁形態 → 種子層覆蓋 → 附著不足 → 銅剝離），**完整鏈現為五環，首環為拋光等級**。➜ **這解釋了玻璃供應商（Corning、Plan Optik、NEG）的地位為何高於「原料商」。** ➜ 並使「Corning small via diameter」空缺的提問方式**第四次修正**：須追加「該數值在何種起始拋光等級（raw/DSP/MDF）下量得」。
  6. ⭐⭐⭐ **側壁粗糙度首次連到電性代價，且孔電阻正在成為 TGV 的驗收指標。** Plan Optik（學術側）：**有效 TGV 導電率低於塊材銅 ➜ 孔電阻上升 ➜ 直接限制 RF 設計彈性；側壁粗糙度直接影響有效導電率**；硼矽 @2.4 GHz εr 4.56 / tan δ 0.0064。**CIT EP4800151A1**（排他權側）：請求項明文 **TGV 上下電阻 ≤0.1 Ω**——該量實為**孔內覆蓋均勻度**（側壁不均會使上下量測值分歧），故單一數字同時承載「通不通」與「勻不勻」。➜ **兩個獨立來源、不同角度，指向同一物理量；本 wiki 的 TGV 驗收項此前皆為幾何與形貌。** ⚠ CIT 未給孔徑/厚度/量測法，**不可與任何其他 TGV 電阻並列比較**。
  7. ⭐⭐⭐ **EMIB-T 首次取得良率數字，且同時揭示一種本 wiki 未見過的風險轉移型態。** 欣興 **2026-07-29 法說會**：EMIB-T「尚未成熟」，三家基板供應商（Unimicron、Ibiden、Shinkawa〔⚠ 疑為 Shinko 轉寫誤植〕）**2027 年底初期量產目標良率僅 50%**；根因為**矽橋直接埋入基板 ⇒ 良率風險自晶圓廠（中介層）換手到基板廠**。⭐⭐⭐ **Intel 對供應商提供「獲利保證機制」**，使其在良率不佳時仍可獲利。➜ 本 wiki 此前記錄的風險轉移**皆為技術性**（ASE/Deca 公差預算下移至微影層、Besi 下移至機構層）；**本件是財務性的第一例**。新候選論述：「**當良率學習曲線的成本落在供應商而收益落在買方時，買方會以合約把該成本買回來。**」⚠ 單一來源，列候選。➜ 客戶首次入庫：**Google 第九代 TPU（2028 量產）、MediaTek 協同設計** ➜ **MediaTek 同時出現在 NVIDIA 與 Google/Intel 兩條 XPU 路線上。** ⚠ 不可與 CoWoS「5.5× 良率 99%」相減（量測邊界與計量對象皆不同）。
  8. ⭐⭐⭐ **面板成本模型新增第一個有量測支撐的邊界項：玻璃載板不是可無限攤提的固定成本。** SemiEng（2026-04-16）：**重複使用之玻璃載板在約 43 µm 處出現崩缺與微缺陷，衝擊韌性較新品明顯下降。** ➜ 2026-09-23 所立之「任何降本 N% 宣稱都要追問會計邊界」常駐規則，此前的邊界項皆為**分析假設**（是否計入 GPU/HBM 晶粒成本）；**本項是第一個由物理劣化支撐者**，且直接影響 Lujan 的淨結論。➜ ⭐⭐⭐ **並與解接合結構選擇耦合**：KIMM 顯示**標準 TBW 結構把殘留物留在玻璃載板（消耗品），反轉結構留在 Si 元件晶圓（產品）**；作者選反轉（因面平滑）＝**選擇把清潔負擔放在產品側**。➜ **「清潔成本 vs 載板壽命」是一組真實交換，不是製程偏好。**
  9. ⭐⭐⭐ **玻璃的成本溢價首見（30–50%），且良率口徑出現衝突。** BOE 完成產線認證（**20 層 9-2-9、>100×100 mm、累計 CNY 13.8 億、2027 H1 量產決策、2028 目標 15,000 片/月**，客戶已進入「實際貼裝 CPU/GPU 於伺服器運行」階段）。同文給出**玻璃相對有機成本溢價 30–50%**。➜ **合併 Lujan 的「0.5 個百分點良率差即完全抹平面板節省」後，結論比任一單獨來源更嚴厲：面板/玻璃不只需追平良率，還需吸收 30–50% 基材溢價。** ⚠⚠ **良率口徑衝突**：本文記玻璃 **60–70%** vs 有機 **80–90%**；本 wiki 既有為 **70–85%** vs **>90%**。**差距（≈20 個百分點）一致、絕對值不一致，本 wiki 不裁定，兩組並列皆標 ⚠**；對 Lujan 容忍度之比值自 10–40 倍**上修為 40–60 倍**。⚠ **溢價的會計邊界未載明，釐清前不得用於任何成本推算。**
  10. ⭐⭐⭐ **玻璃並未消除翹曲，只是把它移到另一個界面——第二例，且翹曲控制手段再下移一層。** **Dongwoo Fine-Chem CN122439424A**：玻璃核心的多層配線基板**仍需表面之有機強化層施加壓縮力**來緩衝玻璃本體受拉之應力。➜ 與 Lau（2026-09-21）「玻璃核心使 micro-bump 應變 9.12%→4.43%（減半），卻使 PCB 側 BGA 應變 8.43%→19%（加倍有餘）」同型。➜ **翹曲控制自「選材料」下移到「預加應力」**，更新形式：「**當幾何與材料都已被上游鎖定，剩下的自由度是在層與層之間分配殘餘應力的正負號。**」前二例（JCET 晶粒尺寸梯度、Absolics RDL 晶粒長寬比之比 0.85–0.99）皆在金屬層，**本例首見有機層**。
  11. ⭐⭐⭐ **「透明性要求」逐層傳播，2026-09-23 之推論鏈延長且首次取得實驗佐證。** 原鏈（自 EVG TW202611995A 推得）為「解接合方式 → 載板須透明 → …」；**KIMM 顯示在反轉結構中，黏著層（C1301）也必須對 308 nm 透明**。➜ **更新鏈：解接合波長 → 載板 *與* 位於釋放層之前的所有層皆須透明 → 材料選擇空間逐層收窄。** 比原推論嚴格一級，且**不再只是本 wiki 推論**。➜ ⭐⭐ 附帶：**220 mJ/cm² 是閾值而非斜率**（低於它，發數自 1 跳到 10，**吞吐降一個數量級**）➜ 「關鍵參數不是單調的」系列新增**第二種型態：階梯（閾值）**，與既有六例的「區間」型態**應分開記載**。
  12. ⭐⭐⭐ **「TGV 的失效在界面與孔緣」須改寫為可工程版本。** Okuno：**剝離後的膜在玻璃側與膜側兩側皆含 Zn ⇒ 破壞面在 ZnO 層內部（內聚破壞），不在界面** ⇒ 界面強度高於黏結層本體。➜ **新形式：「TGV 的失效預設落在界面；把它移回本體，正是黏結層工程要達成的事。」**（非推翻 2026-09-22 第 6 條，而是指出它可工程、非本質。）並為 Corning「賭界面可做牢」一側提供**第一個獨立實驗支持**（但來自另一家公司、另一種化學）。
  13. ⭐⭐ **熱管理新增「整合位置」軸線：D2C（晶片內）／D2P（封裝基板內）／散熱片（封裝外）**，取捨軸為「**越靠近接面熱性能越好，但製程越脫離既有後段產線**」。TU Delft × Nexperia 的 D2P：**625 W/cm²、接面 43 °C（散熱片 78 °C、空氣 220 °C）、COP >10³、免 TIM、傳統後段製程即可**。➜ ⭐⭐⭐ 且揭示一個未記錄的代價：**濕潤面積僅為晶片 footprint 的 78%，其餘 22% 被接合環氧佔去** ➜ **封裝內的冷卻結構不是加上去的，是從別的功能手上要來的面積。** ⚠⚠ 功率電子場景（輸入功率僅 37–41 W），**絕對數值不得外推至 AI 封裝**（本 wiki 記載之 AI 封裝功耗為 600 W → 4,100 W）。
  14. ⭐⭐ **玻璃的適用範圍被第三個獨立來源限縮，且首次來自頻率側。** Plan Optik：**<10 GHz 既有 PCB 方案已足夠；硼矽玻璃適用 <5 GHz，更高頻應改石英。** 前二限縮為 Lam 的 >~100×100 mm、Lujan 的大型複雜封裝。➜ **本 wiki 應避免「玻璃取代有機」的無條件表述。** 附帶旁證：**BOE 選擇 >100×100 mm 面板，即面板陣營自己也把 100×100 mm 當作起點而非上限。**
  15. ⭐⭐ **翹曲論述應拆為「製程翹曲」與「服役期機械負載」兩條線。** Micron US20260282214A1：**貫穿封裝錨（through-package anchors）+ 阻尼裝置** ➜ 加強件自表面貼附改為機械鎖固，並引入**阻尼（動態）**而非僅剛度（靜態）。本 wiki 的翹曲討論此前幾乎全在製程階段。➜ 與 2026-09-22 把「熱」拆為運作熱／製程熱為**同一手法的第二次應用**。
- 專利訊號：本輪五件（+2 件同輪檢出未單獨收錄）集中在**玻璃/TGV 一條主線**，並在其上呈現三個層次。（1）⭐⭐⭐ **Intel 對 liner 做三軸圍籬**：**JP2026119036A 雙 liner（徑向，以第一/第二楊氏模數區分）× US20260136966A1 光聚合物 liner（圖案化手段）× US20260136975A1 部分 liner（軸向，已收）**——三者正好涵蓋 liner 的三個正交自由度，是圍籬式布局的教科書形狀，且落在**單一製程步驟的參數空間**而非產品結構上（對照 SanDisk 2026-05 的產品結構圍籬）。**首見 liner 的力學參數本身被寫進請求項**，使本 wiki 2026-09-23 之「liner 的模數 × CTE 是二維互相拉扯的設計空間」推論取得排他權層級證據。（2）⭐⭐⭐ **CIT EP4800151A1 是本輪唯一的量化請求項（上下電阻 ≤0.1 Ω）**，且是 PVD 深寬比牆三條互斥解法中的一條。（3）⭐⭐⭐ **Dongwoo CN122439424A 把翹曲控制下放到預應力層**。⚠ **除 CIT 外本輪專利全部無量化數值——「專利軌訊號以定性為主」連續第四輪成立**，本輪最強的量化仍全部來自論文軌（IMAPS DPC 2026 全文）。⚠ **兩件同輪檢出未單獨收錄**：Intel **US20260005114A1**（玻璃核心混合面板之框架，**CTE < 11**，Intel 少見的量化請求項）與 **US20260005126A1**（玻璃核心側壁聚合物塗層），已記入 [[entities/intel]] 布局表並列為新空缺。
- 作業面發現（三項，皆影響後續檢索策略）：
  1. ⭐⭐⭐ **IMAPSource 場館掃描的正確 source id 為 `S4210197219`（"IMAPSource Proceedings"），不是 `S4210189957`（"Additional Conferences (Device Packaging HiTEC HiTEN & CICMT)"）。** 後者 1,342 篇但 **2026 年為 0**（最新僅到 2021）。以前者過濾 `from_publication_date:2026-01-01, type:article` 得 **457 篇**，去重後 **452 篇為新**（既有僅 5 篇）、**451 篇有摘要**。➜ **2026-09-23 列為「最高優先」的 IMAPS DPC 2026 整輯系統掃描，本輪完成建立索引並取用 4 篇全文（另 2 篇備用全文已下載）。該池仍有 448 篇未取用，是目前投入產出比最高的來源池。**
  2. ⭐⭐ **OpenAlex 關鍵詞檢索的訊號強度再次遠低於場館導向檢索**（本輪兩組片語檢索合計 84 筆，去重與半導體過濾後僅 20 筆候選、實際採用 1 篇；場館掃描則 4 篇全中）。➜ **2026-09-23 之觀察（「本輪高價值全部來自 IMAPS 直取而非 OpenAlex 關鍵詞檢索」）本輪再次成立** ➜ **下輪配額應明確偏向場館/DOI 前綴導向檢索。** 唯一例外：片語組撈到 `10.2961/jlmn.2026.02.2010`（JLMN 雷射解接合），是本輪對「debonding」空缺唯一的進展來源——**片語檢索仍有不可取代的邊緣價值，不應歸零。**
  3. ⚠⚠ **兩個申請人檢索式本輪被證實無效，原因不同，需分別處置。**
     - **Bruker**（量測軌，已兩輪未動）：`pa="bruker" ...` 命中 673 件，但抓取的 25 件中約 20 件為 **NMR／質譜／空間生物學／X 光繞射**——**Bruker 的封裝量測業務在其生命科學為主的專利組合中被完全稀釋。** ➜ **不應再以申請人檢索 Bruker，須改以技術詞（"bond pad" + "profil*"、"white light interferometry"、"AFM" + "wafer"）或直接以其產品線（Dimension、Contour）文獻檢索。**
     - **Semes**（Samsung 自製 W2W 設備商）：命中件全為**通用 substrate-treating / thermal-treatment apparatus**，**無任何接合案件**。➜ 與 2026-09-23 對 Hanwha Semitech 的發現**同型**（KR 優先案尚未進入 EP/PCT），**不應解讀為訊號偏弱**。
     - **Micron**：`ti,ab="package"` 僅命中 3 件且無 HBM 案 ➜ 與 2026-09-23 之「Amkor 以標題檢索幾乎無效」同型。**下輪改以技術詞（"through silicon via"、"stack"、"hybrid bond"、"microbump"、"base die"）收斂。**
- 下輪建議：
  1. ⭐ **最高優先（延續且路徑已備）：續掃 IMAPS DPC 2026 的剩餘 448 篇。** 索引與評分腳本已於本輪建立（場館 id `S4210197219`；全文路徑 `https://imapsource.org/article/<id>.pdf`，本輪 6/6 下載成功）。**本輪已下載但未取用之兩篇備用全文**：`10.4071/001c.167759`（**ASE：12 吋晶圓級玻璃載板可靠度與強度的新評估方法論**——直指本輪新立之「載板重複使用」空缺）、`10.4071/001c.167496`（**ASU：模封核心基板翹曲建模 + 神經網路最佳化**）。**建議下輪優先取這兩篇，因兩者各自對應本輪新立的最高價值空缺。** 其他高分候選：`167738`（Yole 市場趨勢，kw=33）、`166918`（Cornell Glass-on-Glass）、`167501`（玻璃基 AP 與 CPO）、`167775`（LLO 接合晶圓再利用）、`167490`（直接 Al-Cu 接合）、`167028`（DNP 玻璃上細間距 RDL）。
  2. ⭐ **次高優先：追「濺鍍側 2026 年的深寬比實績」。** 本輪新立之核心論述（「瓶頸在金屬化」）**依賴一篇 2025-09 的舊文作為成孔側對照**，若濺鍍側已改善則幅度需重估。追蹤：AMAT / TEL / Evatec 對高 AR 濺鍍覆蓋率的任何公開數字。
  3. ⭐ **追 TEL 140 nm 載具的電性結果**（作者明列為 next step：多層 140nmP 測試載具）。**在取得電性數據前，本 wiki 對「W2W 已在 140 nm」的引用必須一併陳述「僅接合墊、未電性測試」。**
  4. ⭐ **追 D2W 陣營對「W2W 已 140 nm、D2W 仍 6–9 µm」這 43–64 倍差距的任何公開表態。** 本 wiki 目前一件也沒有。
  5. **專利軌輪替（依本輪發現重排）**：本輪完成 **Micron、Bruker（證實無效）、Semes（證實無 2026 EP 公開）**。**改以技術詞而非申請人**續查：Micron（技術詞）、Amkor（技術詞）、**Ibiden / Shinko / Unimicron**（三者本輪以 EMIB-T 良率關鍵路徑身分出現，**優先序上調**）。**新增輪替對象**：Plan Optik、CIT、Okuno、Dongwoo、Atotech（本輪五個新申請人）。
  6. ⭐ **論文軌應鎖定「面板級載板在 debonding 階段的直接翹曲量測」**——KIMM 只覆蓋了晶圓級且未量翹曲；`167759`（ASE 玻璃載板）與 `167496`（ASU 翹曲建模）是最接近的兩條線索。
  7. 📌 **既有未結案項延續**：`10.1016/j.mssp.2026.111165` 全文（TGV 陣列力學絕對值）、Nature Electronics CPO 綜述全文、CoWoS「5.5× 良率 99%」量測邊界、Samsung US20260247940A1 目標產品線、JCET 韓國團隊產能佐證、Silicon Box 第三方驗證、PHB 量化數據、16-Hi HBM4 對賭驗證、AMAT 混合接合 CMP **市占**、Amkor 使用誰的 TCB 鍵合頭、`info-wmcm.md` 被截斷句子的內容重建、**Samsung 要求 Besi 做的機台設計變更內容**（本輪未取得任何進展）。**Wiley Advanced Science（`10.1002/advs.202512611`）依 2026-09-22 規則已正式降級，本輪未嘗試。**
  8. **lint 待辦**：**沿用**：`glass-substrate.md` 中「玻璃核心基板」與「玻璃核心中介層」混用需分開（**本輪 Okuno 再獲一獨立依據：其 L/S 1/1 µm 目標明確繫於「玻璃中介層」而非核心基板** ➜ 與 Fraunhofer 的「GCS 走 SAP/ABF/PID、中介層走 damascene」一致，**拆頁依據已有兩個獨立來源，優先序再上調**）；`raw/_titles.tsv` 檢出之 **18 組重複 raw 檔**待人工處置（**本輪新增 15 筆未產生任何新重複**）。**新增**：`glass-substrate.md` 已累積大量以日期分段的更新區塊，建議下次整理時按主題重組（TGV 成孔／TGV 金屬化／翹曲與 CTE／廠商與時程四段）。

> **2026-09-24 營運附註（git 鎖檔）**：本輪開始前 `.git/*.lock` 與 `.git/objects/**/tmp_obj_*` **皆為 0**，`git status` 乾淨。刪除權限**仍未開啟**，沿用 `mv` 處置。**依 2026-09-23 之結論，本輪把移鎖函式包在同一個 shell 呼叫內、於每次 git 指令前執行——第一次 commit 即成功，無「Another git process seems to be running」。該作法確認有效，應固定沿用。**
> **2026-09-24 補記（commit 後）**：commit `daafbbf`（**47 檔**，+2,104 / −29）成功。檔案分布：來源摘要 14、技術 5（hybrid-bonding / glass-substrate / foplp / emib / tsv）、概念 3（thermal-management / advanced-packaging-market / test-metrology-packaging）、實體 4（intel / tel / micron / corning）＋ ev-group、raw 15（articles 4／patents 5／papers 6）、`_titles.tsv` 1、`_collected_urls.txt` 1、`index.md`／`log.md`／`overview.md` 各 1。
> **驗證步驟（六項，沿用 2026-09-23 之固定收尾程序並新增兩項）**：①`_collected_urls.txt` 新增 15 筆之檔案路徑**全部存在**；②**15/15** raw 檔 frontmatter 依其軌別（articles/patents/papers）之必要欄位**全部齊備**；③以 `.env` 三個變數值反向 grep `git show HEAD`、`wiki/`、`raw/`、近三次 commit message，**0 次命中（無憑證外洩）**；④`index.md` 宣告數字與實體檔案**完全相符**（總頁數 **635** = `find wiki -name '*.md'`；來源 **580** = `wiki/sources/` 實際檔數）；⑤**新增**：commit 內所有檔案之 UTF-8 合法性檢查，**0 個非法**；⑥**新增**：commit 內所有 wiki 內容頁之 `updated:` 欄位皆為 2026-09-24（`index.md`／`log.md` 無該欄位，前者以 `**更新時間 Updated**` 記於本文並已更新）。
> ⚠ **`index.md` 來源清單重排作業之風險記錄（本輪實際發生並已修復）**：以「僅保留 `- [[sources/` 開頭之行」重建清單時，**誤刪了一個以 `> - [[sources/` 開頭的「補登 / Backfilled」引用區塊（8 行）**。以 `git diff` 比對 `[[sources/...]]` 連結集合後發現並復原，最終確認 **HEAD 有而現版無者 = 0、新增 = 14**。➜ **作業規範新增：任何對 `index.md` 清單的批次重排，完成後必須以連結集合差集（`comm -23`）驗證無遺漏，不可只看行數。**
> **git 鎖檔統計**：本輪產生 **58 個 `tmp_obj_*`** 與 `index.lock`／`HEAD.lock`／`maintenance.lock`，全部以 `mv` 移入 `.git/_stale_tmp_objs/`（`find` 依 2026-09-22 之記載**限定 `.git/objects`**）。該目錄現累計 **1,213 個**檔案。
> **建議（需人工，沿用且已連續七輪）**：（a）於排程任務設定中為 `D:\@source\AdvancedPackaging` 開啟刪除權限；或（b）手動刪除 `.git/_stale_tmp_objs/`。**⚠ 該目錄已達 1,213 檔（本輪 +66），優先序維持在「應處理」。**

---

## [2026-09-25] collect | 每日自動蒐集 — 先進封裝最新動態
- 搜尋查詢數：新聞 **6 組** / 專利 **4 組 CQL** / 論文 **1 組場館掃描（IMAPSource S4210197219，457 筆全量）**
- 成功抓取：**15 篇**（articles **5**, patents **5**, papers **5**, reports 0）
- 失敗/跳過：**取用失敗 1**（Digitimes `a20260922PD212`「TSMC×ASE 高雄先進封裝聚落動土」**403**，依 spec 不重試——該廠區訊息本 wiki 既有記載已涵蓋 kaohsiung 三筆）；**已收錄 8**（SemiEng「making-hybrid-bonding-better」×2、「can-fine-pitch-hybrid-bonding-go-high-volume」、「hybrid-bonding-makes-strides」、「panel-level-packagings-second-wave」、「glass-substrates-gain-momentum」、「flat-enough」、「advanced-packaging-limits」；3DInCites「IFTLE 648」）；**主動棄收（低訊號）2**（3DInCites 2026-08「Breaking the Compute Barrier with Light」——**全文無任何頻寬/pJ per bit/節距數字、無具名公司與時程**，依品質過濾第一條棄收；SemiEng「Advanced Packaging Depends On Materials And Co-Design」2025-05——**無 CTE 值、無 L/S、無翹曲數字**，僅保留 Fraunhofer IZM 之定性觀察）；**低相關約 40**（OPS b2「laser lift-off / debonding」17 件中 14 件為船用複合管焊接、混凝土橋墩裂縫、鈦基複材、固態電池電解質等非封裝命中；b1 前 25 件中約 14 件為靜電吸盤／基板固定裝置／電池隔熱片）
- 資料源狀態：WebSearch ACTIVE ✓ | EPO OPS ACTIVE ✓ | OpenAlex ACTIVE ✓
  - **OPS 僅 5 次呼叫／上限 10**（1 auth + 4 search + **0 detail**）——⭐ **本輪確認 search/biblio 的回應已直接內含 abstract、applicants（含 original 欄）、inventors 與 IPC，四件中三件無需 detail 呼叫即可完成收錄。作業改進：detail 呼叫應改為「僅在 biblio 缺 abstract 時」才發出。** 無 403/429。
  - OpenAlex 全 200，無 429。
- 新增 wiki 頁面：**15 頁**（來源摘要 **14**、實體 **1**）
  - 註：15 個 raw 檔 → 14 頁來源摘要，因 **Ibiden + Shinko 兩件光波導專利合併為一頁**（`2026-09-25_patent_ibiden-shinko-substrate-optical-waveguide`），兩者構成同一產業訊號
  - 新建實體頁：**`wiki/entities/dnp.md`**（列管自 2026-09-15；觸發點：已被 20 頁以上引用 **且** 本輪首度取得其一手量化論文）
- 更新 wiki 頁面：**12 頁**
  - `wiki/technologies/glass-substrate.md`（⭐⭐⭐ TGV 金屬化第四條路線；⭐⭐⭐ 玻璃平坦度→<3 µm L/S；⭐⭐⭐ Cornell 反向論證；⭐⭐⭐ DNP 電遷移；⚠⚠ 濺鍍 AR 空缺未結清）
  - `wiki/technologies/tsv.md`（⭐⭐⭐ σ ~ 1/r、MTTF ~ r²；⭐⭐⭐ 訊號 TGV 的上游成因；⭐⭐ 保守幾何第二來源）
  - `wiki/technologies/foplp.md`（⭐⭐⭐ 玻璃載板強度+翹曲兩端；⚠⚠ 43 µm 修正；⭐⭐⭐ 曝光場 250×250 mm；⭐⭐ 同向改善型；面板六尺寸；Yole 市場；模封核心基板）
  - `wiki/technologies/hybrid-bonding.md`（⭐⭐⭐ W2W/D2W 時間序列與斜率；⭐⭐ 高分子 ILD 排除混合接合；LLO 進入流程）
  - `wiki/technologies/copackaged-optics.md`（⭐⭐⭐ 基板廠首度入場；⭐⭐⭐ 波導住哪一層的三個答案；⚠⚠ Cornell vs imec 並列）
  - `wiki/technologies/emib.md`（⚠⚠ 申請人檢索路徑證實無效）
  - `wiki/concepts/test-metrology-packaging.md`（⭐⭐⭐ 內建驗證結構三實例；⭐⭐⭐ 測試左移第四例；⭐⭐⭐ 三組活化能；EM 驗收口徑）
  - `wiki/concepts/advanced-packaging-market.md`（⭐⭐⭐ Yole 絕對規模；⭐⭐⭐ 載板攤提一手量化；⚠⚠ 43 µm 修正；曝光場稀釋）
  - `wiki/entities/ase-group.md`、`wiki/entities/tel.md`（連續兩輪）、`wiki/entities/ibm.md`、`wiki/entities/jcet.md`、`wiki/entities/intel.md`
  - `wiki/index.md`（頁面總數 635→**650**；來源數 580→**594**；⭐ **依 2026-09-24 新增之作業規範，以連結集合差集驗證：HEAD 有而現版無者 = 0、新增 = 14**）
  - `wiki/overview.md`（6 項結清/降級/修正、11 項新增空缺、**20 條新增或修正之橫向論述**、5 條作業規範）
  - `raw/_titles.tsv`（+15 筆；**新增項未產生任何新的正規化標題重複**；既有 18 組重複仍待人工處置）
- 主要新知識：
  1. ⭐⭐⭐ **本 wiki 上一輪花整輪建立的「TGV 瓶頸在金屬化」論述，本輪被指出可能是一個自找的瓶頸。** **Cornell / FuzeHub** 的推論鏈：**高分子 RDL 因應力只能疊 3–4 層 ⇒ 現行做法是上下各 3–4 層平衡 ⇒ 訊號因此「必須」穿過基板 ⇒ 才需要小孔徑高密度訊號 TGV（~30 µm）**。改用 **SiO₂ damascene RDL**（單面 9–10 層，晶圓廠 BEOL 每天在做）⇒ **訊號 TGV 消失，只剩 ~80 µm 的 power/IO TGV，AR≈10 的牆根本不在路徑上。** ➜ **新橫向論述：「在追問一個瓶頸怎麼突破之前，應先追問它是被哪一個上游選擇製造出來的。」** 並附三個支撐數字：高分子 RDL **CTE ~30–60 vs 玻璃 ~1–3（50×）**；**σ ~ ΔCTE·ΔT·(1/r)**、**MTTF ~ r²**（⇒ 小孔徑本身是平方級的可靠度懲罰，本 wiki 此前只有「越小越好／越小越難」兩個定性方向）。⚠ 立場論文、無自身新量測，且**完全未處理 damascene 在 515×510 mm 面板上的可行性（晶圓廠 BEOL 為 300 mm 晶圓）——此為判定其能否落地的單一最關鍵未知數，已列最高優先空缺。**
  2. ⭐⭐⭐ **RDL 微縮有第二道、且完全獨立的天花板：電遷移。** **DNP（IMAPS DPC 2026）** 以無機介電層+阻障金屬隔開 Cu 與 PID：**Ea 0.9 → >1.23 eV**；**MTTF（2 µm, 2.5E6 A/cm²）0.7 hr → >1000 hr**；B-HAST **120 hr 絕緣破壞 → 200 hr 仍絕緣**；40 GHz／1000 次 TCT 後插入損耗劣化**與線寬無關**；t₀.₁% 外推 **10,000×**。壽命 vs 線寬圖（5 µm 2016 → 0.3 µm 2026）顯示**傳統結構壽命隨線寬崩塌**。➜ **本 wiki 此前所有 RDL 論述皆以 L/S（微影）單軸表達。新形式：「RDL 微縮受兩道獨立天花板限制——上方是微影畫不畫得出來，下方是電流密度撐不撐得住。」** 並由 **Synopsys**（「電流密度已逼近 EM 設計規則上限」）自 EDA 側獨立佐證。⚠ **10,000× 為自「>1000 hr 未失效下限」以 Black's equation 外推，非實測。**
  3. ⭐⭐⭐ **「載板能不能重複使用，取決於分離面落在哪一層。」** 同一輪、兩種基材、結論相反：**ASE**（玻璃載板經歷 fan-out 全製程+化學清洗）**正規化衝擊韌性 0.82 → 0.47（−43%）**、邊緣受損區寬約 43 µm；**TEL**（IR 雷射剝離，分離發生在犧牲層內部）**CSAM 無空洞、可重加工再接合、載板 Rq 1.9–2.3 nm / TTV 11–16 nm、DIW 減 90%**。➜ 與 2026-09-24 之 Okuno「破壞面在 ZnO 層內部（內聚破壞）」為**同一思路的第二個實例：把失效面移到一個消耗性的中間層，是可設計的。** ⚠⚠ **兩者皆僅示範「一次再用」，皆無多輪劣化曲線 ⇒ 載板攤提次數仍不可計算。**
  4. ⚠⚠ **修正：本 wiki 對「43 µm」的語意誤讀（作業規範首次觸發）。** 舊記載（源自 SemiEng 2026-04-16 之轉述）為「重複使用之玻璃載板**在約 43 µm 處**出現崩缺與微缺陷」，讀來是**深度/厚度門檻**；**一手來源（ASE 原文）實為「受損區域寬約 43 µm」。➜ 不存在「43 µm 門檻」，任何以其為前提的推論須撤回。** ➜ **新作業規範：凡單一數字之單位語意係由二手轉述推得者，在取得一手來源前應標 ⚠。**
  5. ⭐⭐⭐ **「混合接合是兩條學習曲線」自單一時點升級為時間序列，且落差正在擴大。** 取得 2022 年錨點（imec **W2W 700 nm**、TSMC SoIC **9 µm**）後：**W2W 研究側 700 → 200 → 140 nm（四年 ~5×）**；**D2W 量產側 9 → 6 µm（四年 ~1.5×）**。➜ **不只有 43–64 倍的落差，斜率也不同。** ⚠⚠ **兩條軌跡各自內部可比、跨軌跡不可比**（一為研究、一為量產）——**此保留今後須與 43–64 倍一併陳述。** ⚠ W2W 三個時點**全部缺電性驗證**。
  6. ⭐⭐⭐ **「先進封裝的每一個新製程步驟，都會很快長出一個內建的驗證結構——因為外部量測跟不上。」** 本輪一次三個同型實例，橫跨學術／排他權／基板：**TEL**（以離子佈植 800–1000 °C 與金屬矽化物相變 200–650 °C 當溫度計，量測奈秒脈衝的瞬態熱；加 ENG Stack 後 ≤200 °C）、**IBM US20260150629A1**（BEOL 內可測試金屬板+測試墊+貫孔，監控雷射解接合）、**Ibiden WO2026186472A1**（波導芯外露部之標靶標記+環繞堤壩+二維碼）。➜ ⭐⭐ **TEL 與 IBM 互補而非競爭**（離線校準 vs 線上監控）——**本 wiki 首次能對同一製程風險列出兩條性質相反的驗證路線。** ➜ **並使「測試左移」取得第四例且型態全新：監控對象是封裝製程步驟而非元件本身。**
  7. ⭐⭐⭐ **一堵牆、四條互不相容的 TGV 金屬化解法（原記三條）。** 新增第四條：**Intel JP2026108527A 的「有機種子材料層」**（放棄無機種子層）。四條分別由研究機構、化學材料商、設備商、IDM 提出。➜ **且②全濕式（Okuno ZnO）與③乾式（超短脈衝雷射成孔 + HiPIMS）的對立，自「材料選擇」升格為「整條製程是濕的還是乾的」⇒ 產線投資結構亦互不相容，互斥性比原記載更強。** ⚠ Intel 案摘要未描述該有機層（僅標題載明），**取得請求項全文前不得與其他三條比較。**
  8. ⭐⭐⭐ **CPO 的排他權戰場首次自晶圓廠／IDM 下移到基板廠，且是兩家同時。** **Ibiden WO2026186472A1（2026-09-10）** 與 **Shinko US20260251844A1（2026-08-27）** 在兩週內各自就「基板／佈線板內的光波導」提出申請。本 wiki 既有 CPO 論述集中於 TSMC COUPE 與 SK hynix，**基板端從未作為獨立排他權主體出現。** ➜ 並與 Cornell、imec 構成**「波導該住在哪一層」的三個答案**（RDL 頂層 / 佈線板本體 / 封裝級高分子波導）。
  9. ⚠⚠ **Cornell 與 imec 在高分子波導上結論相反，本 wiki 並列不裁定。** Cornell：高分子波導（RI 1.6/1.5）約 **5000×3000 nm** vs PIC 之 SiN/SiO₂（1.9/1.45）**500×300 nm**，**10× 尺寸差 = 損耗**，故須改 SiO₂ RDL。**imec/Ghent 實測：以 "Mono" 法設計 SiN taper 相位匹配後，SiN↔高分子耦合接近 1 dB（O-band，TE/TM 皆可），晶片對晶片與晶片對光纖 <2 dB**，且與 chip-first 扇出相容。➜ **兩者不是同層次的矛盾（原理 vs 工程後實測），但政策結論相反。** ➜ 📌 **唯一能分開兩方的實驗：高分子波導在熱循環與吸濕後的耦合損耗漂移**（Cornell 論點的另一半在 imec 資料中完全未被檢驗）——**已列為本輪 CPO 主線追蹤項。** ➜ ⭐⭐ 附帶：**CPO 損耗預算自此至少有兩個獨立環節**（晶粒接合界面 COUPE 0.06 dB / 波導轉接界面 imec ~1 dB），此前本 wiki 只有前者。
  10. ⭐⭐⭐ **曝光場 250×250 mm 上限是「粗快／細慢分工」的物理成因，且它稀釋面板放大的效益。** 步進機曝光場相容尺寸上限 **250×250 mm**，**小於所有已列面板尺寸**（310×310 / 415×510 / 515×510 / 600×600 / 650×650 / 700×700）。➜ **面板面積自 310×310 增至 700×700 為 5.1 倍，曝光場不變 ⇒ 拼接次數同步增加 5.1 倍。** ➜ **任何「面板越大越省」的推算都必須把拼接次數計入。** 並首次取得面板級封裝的絕對市場規模：**Yole 2024 $160 M / 80,000 片（≈330,000 片 300 mm 當量）→ 2030 $650 M / ~220,000 片** ➜ ⚠⚠ **本 wiki 關於面板成本與良率的所有爭論，目前發生在一個 2024 年僅 $160 M 的市場上。**
  11. ⭐⭐ **「翹曲控制手段逐層下移」第四種型態 + 第四類手段。** **JCET CN122497397A**：在**載板本體內以溝槽重分配剛度**（自由區溝槽面積 > 週邊區），前三例（JCET 晶粒梯度、Absolics 長寬比、Dongwoo 有機層預應力）**皆在附加層中分配應力**。➜ **更新形式：「當幾何與材料都被上游鎖定，剩下的自由度是在層與層之間分配殘餘應力；而當連層都鎖定時，剩下的是在同一層內分配剛度。」** 並自 **ASU** 取得**第四類手段：製程排程**（PMC 期間的黏彈鬆弛為主導應力釋放機制）。➜ ⭐ 並與 ASE 方向一致（JCET 把柔性放在晶片之間、剛性留在邊緣；ASE 顯示邊緣正是最脆弱處）⚠ 此一致性為本 wiki 跨文件之觀察，非任一來源之主張。
  12. ⭐⭐ **「關鍵參數不是單調的」新增第三種型態：同向改善。** 既有為**區間**（八例）與**階梯／閾值**（KIMM 220 mJ/cm²）；**TEL 之「加大雷射節距 ⇒ 粗糙度↓ 且 吞吐↑」為第三型。** ➜ **此型是製程最佳化中罕見的「免費午餐」，且其存在暗示當前操作點並非最佳。** ⚠ 未給節距絕對值，不知同向區間何處終止。
  13. ⭐⭐ **「起始表面品質決定下游良率」第二例，作用對象自基板擴展到載板。** Plan Optik（拋光等級，**面**，2026-09-24）+ ASE（邊緣研磨 Rz 1.536 vs 6.497 µm ⇒ 韌性 ~7.0 vs ~1.5，**邊**）➜ **新形式：「玻璃件的機械可靠度由其最後一道機械加工的精細度決定，而非由玻璃本身的材料等級決定。」**
  14. ⭐⭐ **三組電遷移活化能首次可並列定位**：Cu/polyimide RDL **0.9 eV**（DNP）< Cu pillar FCQFN **1.12 ± 0.03 eV**（ASE）< 無機介電覆蓋 RDL **>1.23 eV**（DNP）。⚠⚠ **三者結構、尺度與量測條件皆不同，絕不得作為性能排名。** 但一點成立：**RDL 的兩個端點跨越了銅柱 ⇒「RDL 的 EM 體質不是固定的，取決於介電界面。」** 並首次取得 EM 驗收口徑（**電阻 +100% = 失效、+20% = 最大容許**）➜ **今後引用任何 MTTF 皆須標註失效判準。**
  15. ⭐⭐ **第三條基板路線入庫：模封核心基板（Molded Core，Deca 扇出製程，ASU × SHIELD USA）**——既非有機層壓亦非玻璃，且可嵌入被動與主動元件。路線圖：µbump **25 → sub-10 µm**、RDL L/S **2/2 → 0.5/0.5 µm**、**無 capture pad via 5 → 2 µm**、VIB 貫核節距 **35 → 20 µm**。➜ ⭐ **「無 capture pad via」是本 wiki 首次記載的非線寬型 RDL 微縮項目** ➜ 與 TEL「六角形接合墊佈局造成 Y 向錯位」同屬**「版圖本身是製程變數」的第二例**。⚠⚠ **全篇無翹曲絕對值、無驗證誤差 ⇒ 未能結清翹曲絕對值空缺；本輪反而證實連建模社群自己也未公開絕對值。**
- 專利訊號：本輪五件（含兩件合併記述）呈現**三條線**。（1）⭐⭐⭐ **Intel JP2026108527A 為 TGV 金屬化開出第四條互斥路線（有機種子材料層），並把「玻璃平坦度 → <3 µm L/S 可直接圖案化」寫進申請案**——後者是本 wiki 首次在排他權文件中看到該因果連結，且附數字；同案之**同軸電感置於核心 TGV**與 Cornell 之「被動元件放進 RDL」構成**「封裝正在回收被動元件」的兩個獨立來源、兩種實作層**。（2）⭐⭐⭐ **IBM 與 Ibiden 各自把驗證結構做進產品**（BEOL 內可測試金屬板 / 波導標記+堤壩），與 TEL 論文合為本輪最強的橫向論述之一。（3）⭐⭐ **JCET 以溝槽分配載板剛度**，與 ASE 論文構成玻璃載板的兩端。⚠ **除無**——本輪專利**全部無量化數值**，「專利軌訊號以定性為主」**連續第五輪成立**；本輪最強的量化仍全部來自論文軌（IMAPS DPC 2026 全文）與新聞軌。
- 作業面發現（四項）：
  1. ⭐⭐⭐ **OPS `search/biblio` 已內含 abstract / applicants（含 original 欄）/ inventors / IPC —— 本輪 5 件全部無需 detail 呼叫。** OPS 用量自上輪 6 次降至 **5 次（上限 10）**。➜ **作業改進（固定沿用）：detail 呼叫改為「僅在 biblio 缺 abstract 時」才發出。**
  2. ⚠⚠ **以申請人檢索 Ibiden / Shinko / Unimicron 看不到 EMIB-T。** `(pa=...) and pd within "2026"` 命中 **348 件**，前 25 件**無任何橋接埋入／矽橋／基板腔體案件**，內容集中在光波導、靜電吸盤／基板固定裝置、電池隔熱片。➜ **與 Micron、Amkor、Bruker、Semes 同型，為第三至五個實例。歸納：當目標技術在一家公司的專利組合中只佔極小比例時，申請人檢索必然失效。** ➜ **下輪改技術詞**：`ti,ab="bridge" and ti,ab="embedded"`、`ti,ab="silicon bridge"`、`ti,ab="cavity" and ti,ab="substrate"`、`ti,ab="bridge die"`。
  3. ⭐⭐ **場館導向檢索的訊號密度再次壓倒性優於關鍵詞檢索，連續第三輪成立。** 本輪 **IMAPSource S4210197219（2026 年 457 筆）全量取回並鎖定上輪預選之 8 個 DOI，7/7 全文下載成功（http 200），實際採用 5 篇，全部產出 ⭐⭐ 以上論述。** 本輪**未投入任何片語檢索配額**。⚠ 但 2026-09-24 之保留仍成立（片語檢索有不可取代的邊緣價值），**下輪應恢復 1 組片語檢索以維持覆蓋。**
  4. ⚠ **OpenAlex 的 `institutions` 欄位對 TEL 不可信**：`10.4071/001c.167775` 被解析為「Electoral Commission」「In-Q-Tel」（係將 "TEL Technology Center, America" 與原文誤植之 "Tokyo Election America" 錯配）。➜ **場館掃描取得之機構資訊須以 PDF 原文複核。** 另 `10.4071/001c.167496` 僅列 Arizona State University，PDF 實含 Deca / SHIELD USA 與其他機構。
- 下輪建議：
  1. ⭐ **最高優先：damascene SiO₂ RDL 在面板尺寸上的可行性。** Cornell 整套論證依賴「晶圓廠每天做 10 層 dual damascene」，**但那是 300 mm 晶圓，而 FOPLP 面板為 515×510 至 700×700 mm。** 追蹤：任何在 >300 mm 基材上做無機介電 damascene RDL 的產能、良率或設備資料（DNP、TEL、AMAT、Lam）。**這是判定本輪第 1 條論述能否落地的單一最關鍵未知數。**
  2. ⭐ **次高優先：高分子波導在熱循環與吸濕後的耦合損耗漂移**（分開 Cornell 與 imec 的唯一實驗）。追蹤：imec/Ghent 後續發表、OFC/ECOC 2026、或任何高分子波導的 TCT 後 dB 漂移數據。
  3. ⭐ **續掃 IMAPS DPC 2026 剩餘約 443 篇。** 索引已於本輪重建（場館 id `S4210197219`；全文路徑 `https://imapsource.org/article/<id>.pdf`，本輪 7/7 下載成功）。**本輪已下載但未採用之兩篇**：`10.4071/001c.167738`（**Yole：AI 時代的先進封裝市場趨勢**——可直接餵 `advanced-packaging-market`）、`10.4071/001c.167490`（**Texas A&M：22nm FDSOI 之 Al-Cu 直接接合製程整合，RF 異質整合**——接合軌的新材料體系）。**建議下輪優先取這兩篇。** 其他高分候選：`167501`（玻璃基 AP 與 CPO）。
  4. ⭐ **恢復 1 組 OpenAlex 片語檢索**（本輪為 0），以維持場館掃描涵蓋不到的邊緣。建議片語：`"laser lift-off"|"carrier reuse"|"panel level packaging"` + `type:article`。
  5. **專利軌輪替（依本輪發現重排）**：**改以技術詞追 EMIB-T**（見作業面發現 2）。續查：Micron（技術詞）、Amkor（技術詞）。**新增輪替對象**：TRUMPF（HiPIMS/TGV）、Deca（模封核心基板）、DNP（其 RDL 結構是否有對應專利）。
  6. ⭐ **追 Intel JP2026108527A 之請求項全文**（「有機種子材料層」為第四條路線的唯一來源，摘要未描述該層）；並追**其同族之 US/EP 公開**。
  7. 📌 **既有未結清項延續（本輪無進展）**：`10.1016/j.mssp.2026.111165` 全文（TGV 陣列力學絕對值）、Nature Electronics CPO 綜述全文、CoWoS「5.5× 良率 99%」量測邊界、Samsung US20260247940A1 目標產品線、JCET 韓國團隊產能佐證、Silicon Box 第三方驗證、PHB 量化數據、16-Hi HBM4 對賭驗證、AMAT 混合接合 CMP 市占、Amkor 使用誰的 TCB 鍵合頭、`info-wmcm.md` 被截斷句子重建、Samsung 要求 Besi 做的機台設計變更內容、**D2W 陣營對 W2W 落差的公開表態（仍為零）**、**TEL 140 nm 載具的電性結果**、**2026 年濺鍍側深寬比實績（本輪取得 TRUMPF 為具名對象但無數字）**。
  8. **lint 待辦**：**沿用**：`glass-substrate.md` 中「玻璃核心基板」與「玻璃核心中介層」需拆分（**本輪 DNP 再獲一獨立依據：其同時做 RDL interposer / glass interposer / glass core substrate 三者且各有不同規格** ➜ **拆頁依據已有三個獨立來源，優先序再上調**）；`glass-substrate.md` 已達 **1,350 行以上**且以日期分段，**建議下次整理時按主題重組（TGV 成孔／TGV 金屬化／RDL 與介電／翹曲與 CTE／廠商與時程五段）**；`raw/_titles.tsv` 之 **18 組重複 raw 檔**待人工處置（本輪新增 15 筆**未產生任何新重複**）。**新增**：**評估 `wiki/technologies/glass-carrier.md` 建頁**（本輪已有 ASE + JCET 兩個一手來源，再獲一個即建）。
