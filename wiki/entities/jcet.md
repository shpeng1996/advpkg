...
### 專利訊號：玻璃載板之溝槽分佈設計（抑制封裝過程翹曲）
**CN122497397A**（长电科技管理有限公司；family 100667148，公開 2026-07-31）
發明人：MIAO FUJUN、SHE HAIYAN、ZHANG XIN　IPC：H10W70/05、/614、/68、/692

請求項要旨：玻璃載板承載面分為**中間區域**（含離散晶片放置區與其間之**自由區**）與環繞之**週邊區域**；**溝槽位於載板之內、自自由區延伸至週邊區域，且在自由區之面積大於在週邊區域之面積**；功效為避免封裝過程中載板翹曲。

➜ ⭐⭐ **翹曲控制手段下移的第四種型態**：前三者（**JCET 自身的晶粒尺寸梯度**、Absolics RDL 晶粒長寬比 0.85–0.99、Dongwoo 有機強化層預應力）皆在**附加層中分配應力**；本件是**在載板本體內以幾何（移除材料）重分配剛度**。
  ➜ **JCET 因此在該論述中佔了兩個實例，且分屬兩種型態**（金屬層應力分配 / 本體剛度分配）。
➜ ⭐⭐ **與同輪 ASE 論文構成玻璃載板的「兩端」**（ASE：邊緣強度與重複使用劣化的量測方法；JCET：翹曲的結構設計手段），**且兩者皆來自 OSAT** ➜ 建議玻璃載板升格為獨立技術追蹤對象。
➜ ⭐ 方向一致性：JCET 把柔性放在晶片之間、剛性留在邊緣；ASE 顯示邊緣正是最脆弱處。⚠ **此一致性為本 wiki 跨文件之觀察，非任一來源之主張。**
➜ JCET 在玻璃相關案件上的第二度出現，支持其技術活動**不限於江陰廠的 AI 電源模組與 CPO 定位**。
