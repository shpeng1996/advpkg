...
- **行動裝置**：Apple iPhone 系列——**A20**（iPhone 18，2nm，首款 WMCM 旗艦）；iPad 系列
- **筆記型電腦**：Apple **MacBook M 系列**晶片（2nm 延伸，Apple 2nm 第二大應用）
- **頭戴式裝置**：Apple **R2 晶片**（頭戴裝置，Vision Pro 後繼）
...
- InFO 何時完全退出 Apple 旗艦線？M 系列晶片是否也全面轉 WMCM？（2026-01 消息：M 系列確…

> ⚠ **2026-09-23 修復記錄**：本檔原在約 4,721 位元組處被截斷於一個不完整的 UTF-8 多位元組字元中間，導致全檔非合法 UTF-8（自 2026-09-15 起列為最優先 lint 待辦）。本輪移除該不完整位元組序列，使檔案成為合法 UTF-8。**被截斷的句子內容無法還原，未以任何方式補寫或臆測**；上一行末尾的刪節號即為截斷點。原始語句需自 `raw/` 對應來源檔重建，列為後續待辦。
