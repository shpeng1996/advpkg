...
## HBM4 接合方式確認：維持微凸塊 / Bonding Technology: Confirmed Microbumps

**⭐ 2026-01-13 重要更新（來源：Semiconductor Engineering）**

- **JEDEC 修訂 HBM4 疊層高度上限**：720µm → **775µm**，使 16 層微凸塊接合可行，無需切換混合接合
- **HBM4 接合間距：~10µm microbump**（HBM3E 為 30–20µm）——縮小幅度顯著但仍為微凸塊
- **混合接合推遲至 HBM4E 或 HBM5**（18–20 層疊結構），預計 2028–2029 年以後
- 主因：（1）**成本**：混合接合每封裝更貴，難以競爭；（2）**測試障礙**：混合接合需潔淨表面，而測試探針帶入粒子，「先測後黏」流程極難實現
- 即使使用微凸塊，HBM4 能耗/bit 仍可降低 **30–40%**（混合接合為微凸塊的 1/10，但暫不採用）
- **HBM4 新功能**：客製化 Base Die（AMD/NVIDIA 計畫）、DRFM 防行鎚保護、RAS 改善

*Source: Semiconductor Engineering, Bryon Moyer, 2026-01-13*

---

## Micron HBM4 財務與時程確認 / Micron HBM4 Financial & Timeline Confirmation

**⭐ 2025-12-18 更新（來源：TrendForce）**

- **Micron 2026 全年 HBM 供應 100% 已訂滿**（包含 HBM4）
- **HBM4 2Q26（4–6月）高良率放量**，速度 **>11 Gbps**（超 JEDEC 規格）
- **FY26 CapEx 上調至 USD 200 億**；DRAM+NAND bit 出貨成長 ~20%
- 即使積極擴產，仍只能滿足核心客戶 **50–66%** 需求

*Source: TrendForce, 2025-12-18*

---

## ⭐ Samsung HBM4 ISSCC 2026 技術深度 / Samsung HBM4 ISSCC 2026 Technical Details

*Source: SemiAnalysis, 2026-02-15（引述 Samsung ISSCC 2026 論文）*

Samsung 是三大廠中唯一在 ISSCC 2026 發表 HBM4 技術論文的廠商。關鍵發現：

| 技術指標 | HBM4（Samsung ISSCC 2026）|
|---------|--------------------------|
| Stack 規格 | **36 GB, 12-high, 2048 IO pins** |
| Core die 製程 | Samsung **1c DRAM** |
| Base die 製程 | Samsung **SF4（邏輯製程）** ← 最大架構創新 |
| VDDQ 電壓 | **0.75V**（較 HBM3E −32%）|
| 最高 pin 速度 | **13 Gb/s**（JEDEC 規格 6.4 Gb/s 的 2× 以上）|
| 實測頻寬 | **3.3 TB/s**（JEDEC HBM4 規格 2 TB/s 的 1.65×）|
| 1c 良率（2025）| 約 **50%**（SemiAnalysis 估算）|

### SF4 Base Die 的架構意義

HBM4 的關鍵架構轉型：**Base Die 改用 SF4 邏輯製程**（而非 DRAM 製程）。這帶來：
- 更高操作速度（邏輯製程頻率優勢）
- 更複雜的 PMBIST（Pattern-Based MBIST）：Pattern 觸發式記憶體內建自我測試，可驗證關鍵時序參數（tCCDR、tCCDS），遠優於 HBM3E 的簡單 MBIST
- 代價：SF4 base die 成本高於競爭對手方案；需與 1c DRAM core die 的良率挑戰疊加

### Per-Channel TSV RDQS Timing Auto-Calibration

Samsung 引入「per-channel TSV RDQS timing auto-calibration scheme」——上電後對每個 HBM4 channel 逐一執行校準，補償 Global delay variation（不同 core die 間）和 Local per-channel variation，確保 12 層堆疊中所有 channel 時序對齊。

### SK Hynix HBM4 Base Die：TSMC N12

SemiAnalysis 確認：**SK Hynix HBM4 的 base die 採用 TSMC N12 邏輯製程**（而非 Samsung 的 SF4），是兩家廠商在 HBM4 架構上的關鍵差異。

---

## 爭議與未解問題 / Open Questions

- ~~HBM4 是否採用混合接合？~~ → **確認不採用（2026-01-13 更新）**，維持微凸塊至 HBM4E/HBM5
